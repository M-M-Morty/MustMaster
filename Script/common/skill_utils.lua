...
function SkillUtils.FilterTargets(Targets, Filter)
    local Filtered = UE.TArray(UE.AActor)
    for Ind = 1, Targets:Length() do
        local CurTarget = Targets:Get(Ind)
        if Filter:FilterActor(CurTarget) then Filtered:Add(CurTarget) end
    end

    return Filtered
end

function SkillUtils.IsPlayer(CurActor)
    return CurActor.CharIdentity == Enum.Enum_CharIdentity.Player
end

function SkillUtils.IsBakPlayer(CurActor)
    return CurActor.CharIdentity == Enum.Enum_CharIdentity.Player and CurActor:GetRemoteRole() == UE.ENetRole.ROLE_SimulatedProxy
end

function SkillUtils.IsBoss(CurActor)
    return CurActor.CharIdentity == Enum.Enum_CharIdentity.Monster and
