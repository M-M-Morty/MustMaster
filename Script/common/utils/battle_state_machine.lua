...
function StateBase:turn_to(new_state)
    if self.owner:GetAIServerComponent().bForbiddenBTSwitch == true then
        return
    end
...
function StateBase:update_target(dis)
...
function StateBase:perception_for_alert()
    self.target_cache = self.owner.AIPerceptionComponent:PerceptionForAlert()
    return self.target_cache
end

function StateBase:perception_for_pursue()
    self.target_cache = self.owner.AIPerceptionComponent:PerceptionForPursue()
    return self.target_cache
end
...
function StateInit:tick( ... )
    if self:check_enter_base(StateAlert) and self:perception_for_alert() then
...
function StateAlert:tick()
    self:update_target(self:get_alert_dis())
...
function StateBattlePursue:tick()
    self:update_target(self:get_pursue_dis())
...
function StateFinding:check_enter_battle_pursue()
    ...
    self.target_cache = self:choice_target(self:get_pursue_dis())
    return self.target_cache ~= nil
end
