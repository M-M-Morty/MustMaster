...
function EdUtils:SetOverlapActorOutline(Actor, bShow)
    -- 获取 Overlap 物件高亮
    if not Actor then
        return
    end
    ...
    for Index = 1, OverlappedActors:Length() do
        ...
        if OverActor and OverActor.StaticMeshComponent then
            ...
            if Name == "SmallObjectUnBreakable" then
                if bShow then
                    --OverActor.StaticMeshComponent:SetOverlayMaterial(Actor.AreaAbilityLight)
                    OverActor.StaticMeshComponent:SetRenderCustomDepth(true)
                    OverActor.StaticMeshComponent:SetCustomDepthStencilValue(0)
                else
                    OverActor.StaticMeshComponent:SetRenderCustomDepth(false)
                    --OverActor.StaticMeshComponent:SetOverlayMaterial(nil)
                end
            end
        end
    end
