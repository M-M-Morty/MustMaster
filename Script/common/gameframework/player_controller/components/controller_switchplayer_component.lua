...
function ControllerSwitchPlayerComponent:Input_SwitchPlayer(Idx, UseSuperSkill)
    ...
    self.actor:SwitchPlayer(CharType, false)
end
...
function ControllerSwitchPlayerComponent:BeforeSwitch(OldPlayer, NewPlayer, OrgLocation, OrgRotation)
    OldPlayer:SendMessage("OnRecieveMessageBeforeSwitchOut")    
...
function ControllerSwitchPlayerComponent:AfterSwitch(OldPlayer, NewPlayer, ExtraInfo)
    OldPlayer:SendMessage("OnRecieveMessageAfterSwitchOut")
...
function ControllerSwitchPlayerComponent:StartQTE()
    ...
    local CurPlayer = self.actor:K2_GetPawn()
    local OtherList = {}
    for _, CharType in ipairs(self.QTEAvailablePlayerList) do
        if CharType ~= CurPlayer.CharType then
            table.insert(OtherList, CharType)
        end
    end
    ...
    local NextCharType = OtherList[math.random(#OtherList)]
    local CharInd = self.TeamInfo:Find(NextCharType)
