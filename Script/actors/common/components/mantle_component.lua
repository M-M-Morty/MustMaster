...
function MantleComponent:OnMontageEnded(Montage, bInterrupted)
    ...
    if(self.actor:IsPlayer() and not self.actor.AppearanceComponent:HasMovementInput() and DesiredGait == UE.EHiGait.Sprinting ) then
...
function MantleComponent:OnLand()
    ...
    if self.actor:IsPlayer() and not self.actor.AppearanceComponent:HasMovementInput() then
        self.actor.AppearanceComponent:SprintAction(false)
    end
end
