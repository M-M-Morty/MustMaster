...
function InteractionComponent:BindCalc()
    ...
    for Ind = 1, self.GameplayEffects:Length() do
        local GEObject = NewObject(self.GameplayEffects:Get(Ind), self.actor)
        self.CalcEffectsRef:AddUnique(GEObject)
        self.CalcEffectsHandle:AddUnique(UE.UAbilitySystemBlueprintLibrary.MakeSpecHandle(GEObject, self.actor, self.actor))
    end
