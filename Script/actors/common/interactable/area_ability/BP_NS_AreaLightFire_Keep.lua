...
function M:ReceiveDestroyedEnd()
    for ind=1,#self.OverlapActorIds do
        ...
        self:LogInfo("zsf", "ReceiveDestroyed %s %s %s", G.GetDisplayName(EditorActor), G.GetDisplayName(self), self.eAreaAbility)
        if EditorActor and EditorActor.UseAreaAbility then
            EditorActor:UseAreaAbility(self.eAreaAbility, false)
        end
    end
end
...
function M:OnBeginOverlap_RootSphere(OverlappedComponent, OtherActor, OtherComp, OtherBodyIndex, bFromSweep, SweepResult)
    self:LogInfo("zsf", "OnBeginOverlap_Sphere %s %s", G.GetDisplayName(OtherActor), self.eAreaAbility)
