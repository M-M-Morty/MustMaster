...
function M:DoClientAreaAbilityCopyAction(InvokerActor)
    ...
    if playerActor then
        playerActor.EdRuntimeComponent:Server_AreaAbilityCopy(self)
    end
end
    ...
    if self.ProjectileMovement then
        ...
        utils.DoDelay(self:GetWorld(), self.fAbsorbUpTime, function()
            ...
            if PlayerAtor.GetSocketLocation then
                PlayerLocation = PlayerAtor:GetSocketLocation(PlayerAtor.EdRuntimeComponent.AreaAbilityFlyAttachName)
            end
...
function M:AreaAbility_Fly2Player_OnComponentHit(HitComponent, OtherActor, OtherComp, NormalImpulse, Hit)
    ...
    if self.ProjectileMovement then
        ...
        if ServerPlayerActor then
            local ItemCnt = ServerPlayerActor.EdRuntimeComponent:AddAreaAbilityItem(self.AreaAbilityItemID, 0)
            if ItemCnt > 0 then
                return
            end
            -- 该角色的照明能力++
            ServerPlayerActor.EdRuntimeComponent:AddAreaAbilityItem(self.AreaAbilityItemID, 1)

            self.bFly2Player = false
            self:Multicast_ReceiveAreaAbility(ServerPlayerActor)
        end
    end
end
...
function M:Multicast_AreaAbilityCopy_RPC(InvokerActor)
    ...
    if ServerPlayerActor == PlayerActor then
        ...
        local Montage = ServerPlayerActor.EdRuntimeComponent.AreaAbilityCopyAnim
...
---@param InvokerActor AActor
function M:Multicast_ReceiveAreaAbility_RPC(InvokerActor)
    local ServerPlayerActor = self:GetPlayerActor(InvokerActor)
    if ServerPlayerActor then
...
function M:DoServerAreaAbilityCopyAction(InvokerActor)
    if self:HasAuthority() then
        local PlayerActor = self:GetPlayerActor(InvokerActor)
        if PlayerActor then
            if self.AreaAbilityItemID then
                -- 服务端获取物品; 被copy了能力
                -- 当前物件的能力没有了
                -- 当前是否只是获取一次的区域能力
...
function M:UseAreaAbility(eAreaAbility, bUsing)
    self.Overridden.UseAreaAbility(self, eAreaAbility, bUsing)
end
...
function M:OnBeginOverlap_AreaAbilityTrigger(OverlappedComponent, OtherActor, OtherComp, OtherBodyIndex, bFromSweep, SweepResult)
    if OtherActor.eAreaAbilityMain then
       self:UseAreaAbility(OtherActor.eAreaAbilityMain, true)
    end
end
...
function M:OnEndOverlap_AreaAbilityTrigger(OverlappedComponent, OtherActor, OtherComp, OtherBodyIndex)
    ...
    if not bEffectLasting then
        if OtherActor.eAreaAbilityMain then
            self:UseAreaAbility(OtherActor.eAreaAbilityMain, false)
        end
    end
end
---- Area Ability End -----------
