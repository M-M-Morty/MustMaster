...
function AreaTriggerVolume:FilterClientPlayer(InActor)
    ...
    --G.log:info("[hycoldrain]", "AreaTriggerVolume:FilterClientPlayer--- [%s] [%s] [%s]", G.GetDisplayName(InActor), bClientActor, InActor:IsPlayer())    
    return InActor:IsPlayer() and (bClientActor or UE.UKismetSystemLibrary.IsStandalone(InActor))
end
