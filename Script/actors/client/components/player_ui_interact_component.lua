...
function PlayerUIInteractComponent:OnDialogueEnd()
    ...
    -- if self.CurInteractNpc then
    --     self:Server_InteractNpcEnd(self.CurInteractNpc.Actor:GetActorID())
    -- end
...
decorator.message_receiver()
function PlayerUIInteractComponent:OnRecieveMessageBeforeSwitchOut()
        ...
        for ActorID, NpcInfo in pairs(self.NearbyNpcs) do
            if NpcInfo.bInRange then
                local SelectAction = function()
                    ...
                    self:OnInteractNpcBegin(ActorID)
                end
                local Actor = self:GetActor(ActorID)
                if Actor then
...
-- 开始交互了NPC后, 此处禁止用户其他操作, 播放Camera动画, 播放叙事对话
function PlayerUIInteractComponent:OnInteractNpcBegin(NpcActorID)
    local InteractVM = ViewModelCollection:FindUniqueViewModel(VMDef.UniqueVMInfo.InteractVM.UniqueName)
    if InteractVM then
        InteractVM:CloseInteractSelection()
    end
    ...
    -- 通知NPC进入对话
    -- if NpcActorID ~= nil then
    --     self:Server_InteractNpcBegin(NpcActorID)
    -- end
end
    ...
    for i = 1, #InteractItems do
        ...
        local SelectAction = function()
            ...
            elseif Info.Tpye == NpcInteractDef.Exit then
                ...
                G.log:debug("[MakeSituationItems]", "SituationItem Error Type(%s)", Info.Tpye)
            end
        end
