...
local function OnClickKnapasck(self)
    UIManager:OpenUI(UIDef.UIInfo.UI_Knapsack_Main)
end
...
function WBP_HUD_MainInterface:OnShow()
    self:SetAreaAbilityUsable(false, false)
...
---@param Usable boolean 区域能力可用性
---@param bPlayAnim boolean 播动画
function WBP_HUD_MainInterface:SetAreaAbilityUsable(Usable, bPlayAnim)
    -- if Usable then
    --     self.WBP_HUD_PlayerHP_Item.Switcher_SkillEquip:SetActiveWidgetIndex(1)
    --     if bPlayAnim then
    --         self:PlayAnimation(self.WBP_HUD_PlayerHP_Item.DX_NewSkillGet, 0, 1, UE.EUMGSequencePlayMode.Forward, 1, false)
    --     end
    --     self.WBP_HUD_PlayerHP_Item.WBP_ComBtn_Prop:SetVisibility(UE.ESlateVisibility.Visible)
    -- else
    --     self.WBP_HUD_PlayerHP_Item.Switcher_SkillEquip:SetActiveWidgetIndex(0)
    --     self.WBP_HUD_PlayerHP_Item.WBP_ComBtn_Prop:SetVisibility(UE.ESlateVisibility.HitTestInvisible)
    -- end
end
