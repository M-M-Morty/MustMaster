...
function WBP_CustomLoginWidget:Construct()
    ...
    ClientSubsystem.OnLoginSuccDelegate:Add(self, self.OnLoginSucc)
    ClientSubsystem.OnLoginFailDelegate:Add(self, self.OnLoginFail)
    ClientSubsystem.OnChooseWorldDelegate:Add(self, self.OnChooseWorld)
...
function WBP_CustomLoginWidget:Destruct()
    ...
    local ClientSubsystem = SubsystemUtils.GetTSF4GClientSubsystem(UE.UHiUtilsFunctionLibrary.GetGWorld())
    ClientSubsystem.OnLoginSuccDelegate:Remove(self, self.OnLoginSucc)
    ClientSubsystem.OnLoginFailDelegate:Remove(self, self.OnLoginFail)
    ClientSubsystem.OnChooseWorldDelegate:Remove(self, self.OnChooseWorld)
end

function WBP_CustomLoginWidget:OnLoginSucc(InGuid, InAvatarProxyID)
end

function WBP_CustomLoginWidget:OnLoginFail(InGuid, InErrorCode)
