...
function LGUIManager:ReleaseMouse(releaseMouse)
    ...
    if releaseMouse then
        ...
        UE.UGameplayStatics.SetViewportMouseCaptureMode(G.GameInstance:GetWorld(), UE.EMouseCaptureMode.CapturePermanently_IncludingInitialMouseDown)
...
local function CreateLGUIPrefab(lguiManagerInst, LGUIInfo)
    local LGUIInstance = lguiManagerInst:GetLGUIInstance(LGUIInfo.LGUIName)
    ...
    if not LGUIInstance then
        ...
        actorComponent = prefabActor:GetComponentByClass(LGUIInfo.RootActorComponent)
        if actorComponent then
            lguiManagerInst.tbCreatedLGUI[LGUIInfo.LGUIName] = actorComponent

            actorComponent:SetLGUIInfo(LGUIInfo, prefabActor)
            actorComponent:CallOnCreate()
        end

        ---prefab创建后对子UIWidget进行统一Construct
        local tbUIWidgets = lguiManagerInst:GetUIWdigets(prefabActor):ToTable()
        for _, uiwidget in pairs(tbUIWidgets) do
            local widget = uiwidget:GetWidget()
            if widget and widget.OnConstruct then
                widget:OnConstruct()
            end
        end
...
function LGUIManager:OpenLGUI(LGUIInfo, ...)
    ...
    if actorComponent then
        ...
        table.insert(self.tbShowingLGUI, LGUIInfo.LGUIName)
        if #self.tbShowingLGUI == 1 then
            UIManager.IngameLayerManager:StartShowLGUI()
...
function LGUIManager:CloseLGUI(LGUIInstance, bDestroy)
    ...
    local actorComponent = self.tbCreatedLGUI[LGUIInstance.LGUIInfo.LGUIName]
    if actorComponent then
        --self:ClosePanelWithActor(prefabactor)
        actorComponent:BeginHide(true)
        UE.ULGUIBPLibrary.DestroyActorWithHierarchy(actorComponent.PrefabActor, true)
        self.tbCreatedLGUI[actorComponent.LGUIInfo.LGUIName] = nil
        TableUtil:ArrayRemoveValue(self.tbShowingLGUI, LGUIInstance.LGUIInfo.LGUIName)
        if actorComponent.LGUIInfo then
            if actorComponent.LGUIInfo.UseLguiCamera then
                self:RemoveCamera()
            end
            if actorComponent.LGUIInfo.ReleaseMouse then
                self:ReleaseMouse(false)
            end
        end
        if #self.tbCreatedLGUI then
            UIManager.IngameLayerManager:StopShowLGUI()
        end
    end
end
