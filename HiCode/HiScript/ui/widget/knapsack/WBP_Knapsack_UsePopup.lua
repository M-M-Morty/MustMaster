...
local function OnClickConfirmButton(self)
    ...
    if self.Context and self.Context.TabIndex then
        ItemManager:Server_UseItemByUniqueID(self.Context.TabIndex, self.Item.UniqueID, self.UseCount, 0)
    else
...
function WBP_Knapsack_UsePopup:UseItem(Item,context)
    self.Item = Item
