...
local WBP_Common_Tips_Obtain = Class(UIWindowBase)
...
--end
 ...
 function WBP_Common_Tips_Obtain:SetText(ObjectInfo, TipsContent)
   if not ObjectInfo then
      self.Text_CallingCard:SetVisibility(UE.ESlateVisibility.Collapsed)
   else
      self.Text_CallingCard:SetVisibility(UE.ESlateVisibility.SelfHitTestInvisible)
      self.Text_CallingCard:SetText(ObjectInfo)
   end
   if TipsContent then
      self.Text_Tips:SetText(TipsContent)
   end
 end
