...
---item.itemIcon为item的图标PicKey
---menuIcon为菜单按钮图标PicKey
---若不执行回调，可以通过curItem来取数据
function ui_common_dropdownlist:InitDropDownList(listItemsData, dropDownTitle, selectedCallBack, menuIcon, defaultSelected)
    ...
    if menuIcon then
        PicConst.SetImageBrush(self.Img_MenuIcon, menuIcon)
    else
        self.Img_MenuIcon:SetVisibility(UE.ESlateVisibility.Hidden)
    end
