...
function UIHudHp:OnDiscoverPlayerShow()
    ...
    self.HideFightStateUIHandle = UE.UKismetSystemLibrary.K2_SetTimerDelegate({self,self.HideFightStateUI},3,false)
    self:Emphasize(DistanceState.DiscoverPlayer)
    self:PlayShowHpAnim()
end

function UIHudHp:HideFightStateUI()
    self.Switcher_MosterState:SetVisibility(UE.ESlateVisibility.Collapsed)
    self:EmphasizeEnd(DistanceState.DiscoverPlayer)
    
    UE.UKismetSystemLibrary.K2_ClearAndInvalidateTimerHandle(self, self.HideFightStateUIHandle)
end
