...
local SkillState = require("ui.widget.info.ui_skill_state")
...
local function UpdateBlockSkill(self, Ability, Handle, ActorInfo, ASC)
    local bInstanced
    Ability, bInstanced = G.GetGameplayAbilityFromSpecHandle(ASC, Handle)
...
local function SetSKillImage(self, SkillID)
    ...
    if not SkillID then
        ...
        if SkillData and SkillData[SkillID] then
...
local function RefreshInfo(self)
    ...
    Player = G.GetPlayerCharacter(G.GameInstance:GetWorld(), 0)
    if not Player or not self.SlotIndex then
        return false
    end
    SlotIndex = self.SlotIndex
    ASC = G.GetHiAbilitySystemComponent(Player)
    if not ASC then
        return false
    end
