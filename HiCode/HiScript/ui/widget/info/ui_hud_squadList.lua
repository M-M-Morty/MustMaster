...
local function SwitchPlayerState(self, bToMainPlayer)
    local Avatar = G.GetPlayerCharacter(G.GameInstance:GetWorld(), 0)
    if not Avatar then
...
function UISquadList:OnConstruct()
    self.Btn_Role.Button.OnClicked:Add(self, self.OnClickRoleButton)
    self.Btn_Role.Button.OnHovered:Add(self, OnHoveredButton)
    self.Btn_Role.Button.OnUnhovered:Add(self, OnUnhoveredButton)
    self.Btn_Ranks.Button.OnClicked:Add(self, self.OnClickRanksButton)
    self.Btn_Ranks.Button.OnHovered:Add(self, OnHoveredButton)
    self.Btn_Ranks.Button.OnUnhovered:Add(self, OnUnhoveredButton)
...
function UISquadList:OnDestruct()
    self.Btn_Role.Button.OnClicked:Remove(self, self.OnClickRoleButton)
    self.Btn_Role.Button.OnHovered:Remove(self, OnHoveredButton)
    self.Btn_Role.Button.OnUnhovered:Remove(self, OnUnhoveredButton)
    self.Btn_Ranks.Button.OnClicked:Remove(self, self.OnClickRanksButton)
    self.Btn_Ranks.Button.OnHovered:Remove(self, OnHoveredButton)
    self.Btn_Ranks.Button.OnUnhovered:Remove(self, OnUnhoveredButton)
...
function UISquadList:OnShow(Index, CurSwitchCD, actor)
    ...
    for i = 1, 4 do
        ...
        controller:SendMessage("InitTeamListAttributeSets", i)
    end

    self.IsOnShow = true
end

function UISquadList:InitItemUI(index, attributeSets)
    self.bInitItemUI = true
    self:OnSquadListRoleHealthChanged(index,attributeSets.Health)
    self:OnSquadListSuperPowerChanged(index,attributeSets.SuperPower)
end
...
--角色进入战斗后，处理ui
function UISquadList:OnEnterBattle()
    for i = 1, 4 do
        self["WBP_HUD_SquadList_Item_" .. tostring(i)]:ShowSuperPowerEffect(true)
    end
end

--角色离开战斗后，处理ui
function UISquadList:OnLeaveBattle()
    for i = 1, 4 do
        self["WBP_HUD_SquadList_Item_" .. tostring(i)]:ShowSuperPowerEffect(false)
    end
end
...
---切人逻辑需要重构
function UISquadList:EnableSwitchPlayerUI(bEnable)
    for i = 1, 4 do
        self["WBP_HUD_SquadList_Item_" .. tostring(i)]:EnablePlayerBtn(bEnable)
    end
end
...
function UISquadList:OnSquadListRoleHealthChanged(Index, CurPercent)
    ...
    if self["WBP_HUD_SquadList_Item_" .. tostring(Index)] then
        self["WBP_HUD_SquadList_Item_" .. tostring(Index)]:RoleHealthChanged(CurPercent)
    end
end

function UISquadList:OnSquadListSuperPowerChanged(Index, CurPercent)
    if self.AvatarNum < Index then
        G.log:debug("OnSquadListRoleHealthChanged", "AvatarNum < Index, New: %f, Old: %f", Index)
        return
    end
    if self["WBP_HUD_SquadList_Item_" .. tostring(Index)] then
        self["WBP_HUD_SquadList_Item_" .. tostring(Index)]:SuperPowerChanged(CurPercent)
    end
end
