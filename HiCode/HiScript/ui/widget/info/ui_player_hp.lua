...
function UIPlayerHP:SetHpData(level, dyingLimit, curHealth, healthLimit)
    self.Level = level
    self.DyingLimit = healthLimit * UIConstData.BLOOD_CRISIS_RATIO.FloatValue -- 比例从常量表中读取
...
function UIPlayerHP:EnterVehicleState(bEnterVehicle)
    if bEnterVehicle then
        self.Cvs_Item_Unuse:SetVisibility(UE.ESlateVisibility.SelfHitTestInvisible)
    else
        self.Cvs_Item_Unuse:SetVisibility(UE.ESlateVisibility.Collapsed)
    end
end
