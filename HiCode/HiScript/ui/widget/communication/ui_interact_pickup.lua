...
function M:UpdateParams(SelectionItems, InteractUIType, InteractItemType)
    ...
    G.log:debug('zys', table.concat({'ui_interact_pickup:OnShow()', debug.traceback()}))
...
function M:OnPickupCanceled()
    G.log:debug("OnPickupCanceled", "OnPickupCanceled")
    if self.Pause == true then
        G.log:debug("OnPickupCanceled return", "OnPickupCanceled return")
        return
    end
...
function M:OnPickupCompleted()
    G.log:debug("OnPickupCompleted", "OnPickupCompleted")

    if self.Pause == true and not self.InteractItemType == InteractVM.InteractItemType.Once then
        G.log:debug("OnPickupCompleted return", "OnPickupCompleted return")
        return
    end
