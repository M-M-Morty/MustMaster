...
local function RequireLib()
    if IRPC then
        return
    end
    IRPC = require("micro_service.irpc.irpc")
    PROTOC = require("micro_service.ProtocInstance")
    IRPCLog = require("micro_service.irpc.irpc_log")
    IRPCCore = require("irpc_core")
end


local function DefaultAsyncCallback(FuncName, ClientCtx, Response)
    local Status = ClientCtx:GetStatus()
    if not Status:OK() then
        G.log:info("RPCServiceClientStub",
                "RPCServiceClientStub: default callback of %s. error:%s", FuncName,
                utils.IRPCStatusString(Status)
        )
    end
end
...
function RPCServiceClientStub:AsyncCallRPCFunction(FuncName, ClientContext, FuncRequest, Callback)
    ...
    if not Callback then
        Callback = function(ClientCtx, Response)
            DefaultAsyncCallback(FuncName, ClientCtx, Response)
        end
    end
...
--        PlayerID = 20000
--    }
--    local bResult, ClientStatus = AsyncOfficeRPCStub:Hello(Hello, HelloRequest, AsyncCallback)
...
--        local bResult, Response, ClientStatus = OfficeRPCStub:Coro_Hello(HelloRequest)
--        if not bResult or not ClientStatus:OK() then
