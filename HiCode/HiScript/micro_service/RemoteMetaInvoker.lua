...
function RemoteMetaInvokerMeta.DefaultCallback(RemoteMetaDataInvoker, FuncName, ClientCtx, Response)
    ...
    if not Status:OK() then
        G.log:info("RemoteMetaInvoker",
                "RemoteMetaInvoker: default callback of %s. error:%s", FuncName,
                StatusString(Status)
        )
    end
end
...
--local function Exp1(User, _, UseParam, ExtraData)
...
----example2
-----@param User PlayerState
--local function Exp2(User, _, UseParam, ExtraData)
--    local RemoteMetaInvoker = require("micro_service.RemoteMetaInvoker")
--    local GenericOfficeInvoker = RemoteMetaInvoker.CreateGenericInvoker(MSConfig.MSOfficeEntityMetaName, OfficeGid)
--    local Callback = function(ClientContext, Response)
...
--        end
--    end
--    GenericOfficeInvoker:ExampleMethod(Req, Callback)
--end