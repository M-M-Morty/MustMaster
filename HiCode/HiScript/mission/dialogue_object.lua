    ...
    for _, BranchID in ipairs(self.BranchIDs) do
        ...
        elseif BranchData.Tpye == NpcInteractDef.UIEvent then
            ...
            Callback = function ()
                ...
                if BranchData.IsHideNPC == NpcInteractDef.HideNPC then
                    local PlayerController = SubsystemUtils.GetMutableActorSubSystem(self.DialogueObject.DialogueComponent):GetHostPlayerController()
                    PlayerController.NPCHideComponent:HideInteractNPC()
                elseif BranchData.IsHideNPC == NpcInteractDef.HideRangeNPC then
                    local PlayerController = SubsystemUtils.GetMutableActorSubSystem(self.DialogueObject.DialogueComponent):GetHostPlayerController()
        ...
        local Item = NpcInteractItemModule.DefaultDialogueItem.new(BranchData.DialogueId, BranchData.Name, Callback)
...
function DialogueStepDisplayBranch:ctor(BranchIDs, ChoiceIndex)
    ...
    for _, BranchID in ipairs(self.BranchIDs) do
        local BranchData = NpcInteractTable[BranchID]
        local Item = NpcInteractItemModule.DefaultDialogueItem.new(BranchData.DialogueId, BranchData.Name, nil)
        table.insert(self.Items, Item)
    end
end
