...
function t.gmopenbag()
    ...
    local UIDef = require('CP0032305_GH.Script.ui.ui_define')
    ---@type WBP_Knapsack_UsePopup
    local SecondConfirmWidget = UIManager:OpenUI(UIDef.UIInfo.UI_Knapsack_Main2)
...
function t.gmaddallitems()
    if UE.UHiUtilsFunctionLibrary.IsServerWorld() then
        local Player = p
        local ItemManager  = Player.PlayerState.ItemManager
        local ItemUtil = require("common.item.ItemUtil")
...
function t.GetGameTime()
    ...
    G.log:debug("GetGameTime", "DayNum=%s, HourOfDay=%s, MinuteOfDay=%s", Component.DayNum, 
        math.floor(Component.MinuteOfDay / TimeUtil.MINUTES_PER_HOUR), Component.MinuteOfDay)
end
...
decorator.lua_console_cmd(ClientOnly)
function t.TestAddDesignScheme()
    local MsConfig = require("micro_service.ms_config")
    local RPCStubFactory = require("micro_service.rpc_stub_factory")
    ...
    RPCClient:AddOnlineDesignScheme({
        ...
        DecorationActorInfos = {}
    }, function(Context, Response)
        if Context:GetStatus():OK() then
            G.log:info("yongzyzhang", "TestAddDesignScheme Gid:%s", Response.SchemeGid)
        else
            G.log:info("yongzyzhang", "TestAddDesignScheme failed:%s", utils.IRPCStatusString(Context:GetStatus()))
        end
    end)
end

decorator.lua_console_cmd(ClientOnly)
function t.TestDelDesignScheme(SchemeGid)
    local MsConfig = require("micro_service.ms_config")
    local RPCStubFactory = require("micro_service.rpc_stub_factory")
    local RPCClient = RPCStubFactory:GetRPCStub(MsConfig.OfficeDesignShareServiceName)
    RPCClient:DelOnlineDesignScheme({
        ...
        if Context:GetStatus():OK() then
            G.log:info("yongzyzhang", "TestDelDesignScheme succeed")
        else
            G.log:info("yongzyzhang", "TestDelDesignScheme failed:%s", utils.IRPCStatusString(Context:GetStatus()))
        end
    end)
end

decorator.lua_console_cmd(ClientOnly)
function t.TestUStruct2Table()

    ---@type OfficeManager
    local OfficeManager = UE.UHiGlobalActorLibrary.GetGlobalActorByName(GlobalActorConst.OfficeManager)
    --local GameplayEntitySubsystem = SubsystemUtils.GetGameplayEntitySubsystem(OfficeManager)

    local DecorateHandlerComp = OfficeManager:GetDecorationHandlerComp()
    for _, DecorationInfo in pairs(DecorateHandlerComp.SavedActorDecorationInfos:ToTable()) do
        local StructTable  = DecorationInfo:ToMessageTable("HiGame.Office.OfficeDecorationActorInfo")
        G.log:info("yongzyzhang", "TestUStruct2Table table content:%s", utils.TableToString(StructTable))
        --local StructMessage  = DecorationInfo:ToProtobufMessage("HiGame.Office.OfficeDecorationActorInfo")
        --G.log:info("yongzyzhang", "TestUStruct2Table StructMessage content:%s", StructMessage:DebugString())

        local TargetNewStruct = Struct.BPS_OfficeDecorationInfo()
        TargetNewStruct:CopyFromMessageTable("HiGame.Office.OfficeDecorationActorInfo", StructTable)
        local ProtobufMessage = TargetNewStruct:ToProtobufMessage("HiGame.Office.OfficeDecorationActorInfo")
        G.log:info("yongzyzhang", "TestUStruct2Table New Message after CopyFromMessageTable and transform :%s", ProtobufMessage:DebugString())

        local TargetNewStruct2 = Struct.BPS_OfficeDecorationInfo()
        TargetNewStruct2:CopyFromCProtobufMessage(ProtobufMessage)
        local ProtobufMessage2 = TargetNewStruct:ToProtobufMessage("HiGame.Office.OfficeDecorationActorInfo")
        G.log:info("yongzyzhang", "TestUStruct2Table New Message after CopyFromCProtobufMessage and transform :%s", ProtobufMessage2:DebugString())
