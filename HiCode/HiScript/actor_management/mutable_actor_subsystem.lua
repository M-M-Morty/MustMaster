...
function MutableActorSubsystem:ProcessNewEditorActors()
    ...
    for Ind = 1, Result:Length() do
        ...
        if bShouldAdd then
            ...
            G.log:debug("xaelpeng", "MutableActorSubsystem:ProcessNewEditorActors Actor Ignored:%s", ActorID)
...
function MutableActorSubsystem:SpawnSavedEditorActor(ActorID, JsonWrapper, PropertyMessage)
    ...
    local ExtraSpawnOp = function(Actor)
        ...
        self:AddDDSComponentToActor(Actor)
    end
    local Transform = nil
...
function MutableActorSubsystem:AddDDSComponentToActor(Actor)
    Actor:AddComponentByClass(UE.UDistributedDSComponent, false, UE.FTransform.Identity, false)
    -- Actor:AddComponentByClass(UE.UGeoAssociateComponent, false, UE.FTransform.Identity, false)
end
...
function MutableActorSubsystem:SyncMissionRecords(ActorID, MissionRecordList)
    local MissionAvatarComponent = self:GetMissionAvatarComponentByID(ActorID)
    if MissionAvatarComponent ~= nil then
        MissionAvatarComponent:SyncMissionRecords(MissionRecordList)
    end
end
...
function MutableActorSubsystem:GetHostPlayer()
    if UE.UHiUtilsFunctionLibrary.IsServerWorld() then
        ...
        return G.GetPlayerCharacter(self, 0)
    else
        return G.GetPlayerCharacter(self, 0)
    end
end
