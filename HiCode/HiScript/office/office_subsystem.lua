...
local function OnAutoUseOfficeExpItem(User, _, _, ExtraData)
    if UE.UHiUtilsFunctionLibrary.IsLocalAdapter() then
        G.log:info("yongzyzhang", "OnAutoUseOfficeExpItem failed on LocalAdapter")
        return
    end
...
local function OnUseOfficeFurnitureItem(User, _, UseParam, ExtraData)
    if UE.UHiUtilsFunctionLibrary.IsLocalAdapter() then
        G.log:info("yongzyzhang", "OnUseOfficeFurnitureItem failed on LocalAdapter")
        return
    end
...
function OfficeSubsystem:PostInitializeScript()
    ...
    if bIsServer then
        ...
        self.FurnitureGroupDefaultColorsCache = {}
    end
...
function OfficeSubsystem:OnPlayerEnterOffice(PlayerState)
    ...
    if bIsServer then
        if not self.OfficeManager then
            self.OfficeManager = UE.UHiGlobalActorLibrary.GetGlobalActorByName(GlobalActorConst.OfficeManager, self)
        end
        ---@type OfficeManager
...
function OfficeSubsystem:OnOfficeWorldReady(OfficeManager)
    ...
    if bIsServer then
        ...
        --local PlayerState = UE.UGameplayStatics.GetPlayerController(self, 0).PlayerState
    end
end
...
---获取家具的初始皮肤-染色数据
function OfficeSubsystem:GetFurnitureDefaultDecorationInfo(Actor)
    local PathName = tostring(UE.UKismetSystemLibrary.GetPathName(Actor:GetClass()))
    -- 去掉_C
    PathName = PathName:sub(1,-3)
    ---@type FOfficeDecorationActorInfo
    local DefaultDecorationInfo = Struct.BPS_OfficeDecorationInfo()
    DefaultDecorationInfo.ActorID = Actor.ActorID

    for _, RowName in pairs(self:GetOfficeDataTableRowNames():ToTable()) do
        local SkinDTConfig = self:GetOfficeDataTableRow(RowName)
        if SkinDTConfig and SkinDTConfig.BP and tostring(SkinDTConfig.BP) == PathName then
            local ModelConfig = self:GetOfficeModelConfig(RowName)
            if not ModelConfig.IsBasicMesh then
                local BasicMeshRowName = self:GetBasicModelIDBySkinID(RowName)
                DefaultDecorationInfo.BasicModelKey = BasicMeshRowName
                SkinDTConfig = self:GetOfficeDataTableRow(BasicMeshRowName)
            else
    ...
    DefaultDecorationInfo.Transform = Actor:GetTransform()
    DefaultDecorationInfo.Transform.Scale3D = UE.FVector(1.0, 1.0, 1.0)
    return DefaultDecorationInfo
end

function OfficeSubsystem:GetModelCompDefaultColors(ModelID, ActorID)
    if self.FurnitureGroupDefaultColorsCache[ModelID] then
        return self.FurnitureGroupDefaultColorsCache[ModelID]
    end
    ...
    local ModelBPClass = UE.UClass.Load(ActorBPClassName)
    -- 不是家具组
    if not UE.UKismetMathLibrary.ClassIsChildOf(ModelBPClass, UE.AOfficeBasicFurnitureGroup) then
        local NewMaterialName = tostring(SkinDTConfig.Material)
        return self:GetMaterialDefaultColors(NewMaterialName)
    end
    
    local ObjectRegistryWorldSubsystem = UE.USubsystemBlueprintLibrary.GetWorldSubsystem(self, UE.UObjectRegistryWorldSubsystem)
    local FurnitureGroupActor = ObjectRegistryWorldSubsystem:FindObject(ActorID)
    local FurnitureGroupDefaultColors = FurnitureGroupActor:GetFurnitureGroupDefaultColors()
    self.FurnitureGroupDefaultColorsCache[ModelID] = FurnitureGroupDefaultColors
    return FurnitureGroupDefaultColors
end

function OfficeSubsystem:GetMaterialDefaultColors(MaterialName)
    local MaterialDefaultColors = {}
    local MaterialInstance = UE.UObject.Load(MaterialName)
...
function OfficeSubsystem:GetOfficeDesignSchemeOnSlot(Slot)
    ---@type OfficeManager
    ...
    if OfficeManager then
        return OfficeManager:GetDecorationHandlerComp():GetSelfDesignSchemeOnSlot(Slot)
    else
        --TODO 不在事务所也能获取到自己的装修方案
    end
end

--client 发布自身装修方案到装修圈
function OfficeSubsystem:ReleaseDesignScheme(Slot, Callback)
    local RPCClient = self:GetDesignShareServiceClient()
    local DesignScheme = self:GetOfficeDesignSchemeOnSlot(Slot)
    local Desc = DesignScheme.Description
    local DesignActorDecorationInfos = DesignScheme.DesignScheme.DesignActorDecorationInfos
    local DesignActorDecorationInfoTableList = {}
    ...
    RPCClient:AddOnlineDesignScheme({
        Description = Desc,
        DesignActorDecorationInfos = DesignActorDecorationInfoTableList
