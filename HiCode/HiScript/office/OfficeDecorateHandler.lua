...
---@field TrialDecorationItems Struct.BPS_OfficeDecorationTrialItems 非Replicate&存盘 装修模式中的Actor试用项
...
OfficeDecorateHandler.EventNameDict = EventNameDict
...
function OfficeDecorateHandler:ReceiveBeginPlay()
    ...
    GameEventBus.GetEventBusInstance(self):RegisterEventListener(GameEventBus.EventType.OnNotifyOfficeAsset, self, self.ClientOnSyncOfficeAsset)
end
...
function OfficeDecorateHandler:FlushAllDecorationActors(EventParams)
    ...
    G.log:info("yongzyzhang", "OfficeDecorateHandler:FlushAllDecorationActors ActorInfo count:%d current mode:%s", 
...
function OfficeDecorateHandler:unregisterShopCarRefreshEvent(Listener, Callback)
    self.ClientEventDispatcher:RemoveListener(EventNameDict.ClientShopCarRefreshEventName, Listener, Callback)
end
...
function OfficeDecorateHandler:RegisterEvent(EventType, Listener, Callback)
    if not EventNameDict[EventType] then
        G.log:info("yongzyzhang", "OfficeDecorateHandler:RegisterEvent invalid EventType:%s", EventType)
        return
    end
    local InnerCallback = function(...)
        local PackArgs = {...}
        xpcall(function()
            ...
            if errobj then
                G.log:error("yongzyzhang" ,"OfficeDecorateHandler Event Listerner Callback Error:" .. errobj .. debug.traceback())
            else
                G.log:error("yongzyzhang" ,"OfficeDecorateHandler Event Listerner Callback Error:" .. debug.traceback())
            end
        end)
    end
    local CallbackObj = {
    ...
    self.ClientEventDispatcher:AddListener(EventNameDict[EventType], Listener, CallbackObj)
end

function OfficeDecorateHandler:UnregisterEvent(EventType, Listener, Callback)
    self.ClientEventDispatcher:RemoveListener(EventNameDict[EventType], Listener, Callback)
end
...
function OfficeDecorateHandler:GetOfficeOwnerPlayer()
    if UE.UHiUtilsFunctionLibrary.IsServer(self) then
        ...
        return UE.UGameplayStatics.GetPlayerController(self, 0)
    end
end
...
function OfficeDecorateHandler:GetActorDecorationInfo(ActorID, bCreateCopy)
    if self.bInDecorationMode and UE.UHiUtilsFunctionLibrary.IsClient(self) then
        ...
        if self:GetOfficeSubsystem():IsOfficeDecorationActorValid(ActorID) then
            return self:GetActorDefaultDecorationInfo(ActorID)
        end
    else
...
function OfficeDecorateHandler:ResetActorDecorationInfo(ActorID)
    ...
    if UE.UHiUtilsFunctionLibrary.IsClient(self) then
        ...
        self.ClientActorDecorationInfoTable[ActorID] = Default
        ...
        local SavedActorDecorationInfos = {
            {
                ...
                BasicModelKey = "Office_Test_Group",
...
function OfficeDecorateHandler:GetModelUnlockedColors(ModelID, CompIndex)
    ...
    if not ModelConfig.IsBasicMesh then
        ...
        if BasicModelAsset and BasicModelAsset.SkinAsset and BasicModelAsset.SkinAsset[ModelID] then
            ...
            if SkinAsset.ComponentAsset and SkinAsset.ComponentAsset[CompIndex] then
                return SkinAsset.ComponentAsset[CompIndex].UnlockedColor or {}
            end
        end
    else
        local BasicModeAsset = self:GetOfficeBasicModelAsset(ModelID)
        if BasicModeAsset and BasicModeAsset.ComponentAsset and BasicModeAsset.ComponentAsset[CompIndex] then
            return BasicModeAsset.ComponentAsset[CompIndex].UnlockedColor or {}
        end
    end
    return {}
end
...
function OfficeDecorateHandler:Server_EnterDecorationMode_RPC()
    ...
    self:Client_EnterDecorationModeResult(OfficeEnums.DecorationErrorCode.OK, "", self.TrialDecorationItems)

end

function OfficeDecorateHandler:Client_EnterDecorationModeResult_RPC(ErrorCode, Msg, TrialItems)
    if ErrorCode ~= OfficeEnums.DecorationErrorCode.OK then
        G.log:info("yongzyzhang", "Client_EnterDecorationModeResult failed, code:%d msg:%s", ErrorCode, Msg)
        return
    end
    
    self.TrialDecorationItems = TrialItems
    
    --客户端请求服务，拿一份最新的资产数据
    self:GetOfficeSubsystem():RequestOfficeAsset(self:GetOfficeOwnerPlayer(), function(Ok, OfficeAsset) 
        self:ClientOnSyncOfficeAsset(OfficeAsset)
...
function OfficeDecorateHandler:RequestBuyShopCarItems(ShopCarItems, Callback)
    ...
    for _, BuyColorItem in pairs(BuyColorItemTable) do
        table.insert(Req.ColorPayItems, { ModelID = BuyColorItem.ModelKey,
                                          CompIndex = BuyColorItem.Index, Color = utils.FColorToTable(BuyColorItem.Color) })
    end
...
function OfficeDecorateHandler:Server_LeaveDecorationMode_RPC(LeaveParam)
    ...
    local LeaveLogic = function(ErrorCode, Msg)
        ...
        G.log:warn("yongzyzhang", "Server_LeaveDecorationMode_RPC  ActorCount:%d", ActorDecorationInfos:Length())
    ...
    if LeaveParam.bPayAll then
        ...
        self:GetOfficeSubsystem():RequestOfficeAsset(self:GetOfficeOwnerPlayer(),  function(Ok, OfficeAsset)
            self:EndDecorationRPCWrapper()
            if Ok then
                self.OfficeAssetLocalCache = OfficeAsset
            end
...
function OfficeDecorateHandler:Client_LeaveDecorationModeResult_RPC(ErrorCode, ErrorMsg, NeedDeleteActorIDList)
    if ErrorCode == 0 then
        G.log:info("yongzyzhang", "Client_LeaveDecorationModeResult_RPC succeed")
    else
        G.log:info("yongzyzhang", "Client_LeaveDecorationModeResult_RPC failed, code:%s msg:%s")
        return
    end
    ...
    self:unregisterShopCarRefreshEvent()
end
...
function OfficeDecorateHandler:Server_EditorSchemeDesc_RPC(Slot, SchemeDesc)
    local DesignSchemeSlot = self:GetSelfDesignSchemeOnSlot(Slot)
    if DesignSchemeSlot == nil then
        return
    end
    DesignSchemeSlot.Description:CopyFrom(SchemeDesc)
end
...
function OfficeDecorateHandler:GetSelfDesignSchemeOnSlot(Slot)
    for Index, DesignSchemeSlot in pairs(self.DesignSchemeSlots:ToTable()) do
        if DesignSchemeSlot.Slot == Slot then
            return self.DesignSchemeSlots:GetRef(Index)
        end
    end
end
