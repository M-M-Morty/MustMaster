...
function SkillObj:CanActivate()
    ...
    if not CanActivate then
        ...
        if FailTags then
            local FailTagNum = UE.UBlueprintGameplayTagLibrary.GetNumGameplayTagsInContainer(FailTags)
            for Ind = 1, FailTagNum do
                local FailTag = FailTags.GameplayTags:Get(Ind)
                G.log:debug(self.__TAG__, "Ability can not activate right now, SkillID: %d, FailTag: %s", self.SkillID, UE.UBlueprintGameplayTagLibrary.GetTagName(FailTag))
            end
        end
