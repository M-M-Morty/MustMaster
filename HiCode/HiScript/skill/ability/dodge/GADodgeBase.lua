...
---@type GA_DodgeBase_C
local GADodgeBase = Class(GAMulti)

GADodgeBase.__replicates = {
}

---@type UAnimMontage
function GADodgeBase:GetPlayMontage() end

---@type BPA_AvatarBase_C
function GADodgeBase:GetOwner()
    local OwnerActor = self:GetOwningActorFromActorInfo()
    return OwnerActor
end


function GADodgeBase:K2_GetCostGameplayEffect()
...
function GADodgeBase:HandleActivateAbility()
    Super(GADodgeBase).HandleActivateAbility(self)
...
function GADodgeBase:OnExtremeDodge()
...
function GADodgeBase:OnMontageCompleted()
    G.log:info("yb", "montage finish")
    self:K2_EndAbility()
 end
UE.DistributedDSLua.RegisterFunction("OnMontageCompleted", GADodgeBase.OnMontageCompleted)

function GADodgeBase:OnMontageCancelled() 
    G.log:info("yb", "montage cancel")
    self:K2_EndAbility() 
end
UE.DistributedDSLua.RegisterFunction("OnMontageCancelled", GADodgeBase.OnMontageCancelled)

function GADodgeBase:OnMontageInterrupted()
    G.log:info("yb", "montage finish")
    self:K2_EndAbility()
end
UE.DistributedDSLua.RegisterFunction("OnMontageInterrupted", GADodgeBase.OnMontageInterrupted)

function GADodgeBase:OnMontageBlendOut()
    G.log:info("yb", "montage blend out")
    self:K2_EndAbility()
 end
UE.DistributedDSLua.RegisterFunction("OnMontageBlendOut", GADodgeBase.OnMontageBlendOut)

function GADodgeBase:K2_OnEndAbility()
    ...
    Super(GADodgeBase).K2_OnEndAbility(self)
end

---@return UAnimMontage
function GADodgeBase:GetDodgeMontage()
...
function GADodgeBase:ClearSpeed() 
end

UE.DistributedDSLua.RegisterCustomClass("GADodgeBase", GADodgeBase, GAMulti)
