...
function GASkillBase:K2_PostTransfer()
    Super(GASkillBase).K2_PostTransfer(self)
end
...
function GASkillBase:OnCalcEvent(Payload)
    ...
    -- TODO 打开预测
    if not self.OwnerActor:HasAuthority() then
        return
    end

    G.log:debug(self.__TAG__, "OnCalcEvent %s, bIsBindType: %s, bBind: %s, Role: %d", GetTagName(EventTag), KnockInfo.bIsBindType, KnockInfo.bBind, self.OwnerActor:GetLocalRole())
...
function GASkillBase:CanCalc()
    if self.OwnerActor:HasAuthority() then
        return true
    end
...
UE.DistributedDSLua.RegisterCustomClass("GASkillBase", GASkillBase, GABase)
