...
local GARush = Class(GAPlayerBase)

GARush.__replicates = {
    Projectiles = 0,
}

function GARush:K2_PostTransfer()
    Super(GARush).K2_PostTransfer(self)
end
...
UE.DistributedDSLua.RegisterFunction("OnStartMontageCompleted", GARush.OnStartMontageCompleted)

function GARush:OnStartMontageBlendOut()
    self:PlayLoopMontage()
end
UE.DistributedDSLua.RegisterFunction("OnStartMontageBlendOut", GARush.OnStartMontageBlendOut)

function GARush:OnStartMontageInterrupted()
    self:K2_EndAbility()
end
UE.DistributedDSLua.RegisterFunction("OnStartMontageInterrupted", GARush.OnStartMontageInterrupted)

function GARush:OnStartMontageCancelled()
    self:K2_EndAbility()
end
UE.DistributedDSLua.RegisterFunction("OnStartMontageCancelled", GARush.OnStartMontageCancelled)
...
UE.DistributedDSLua.RegisterFunction("OnLoopMontageCompleted", GARush.OnLoopMontageCompleted)

function GARush:OnLoopMontageBlendOut()
    self:PlayEndMontage()
end
UE.DistributedDSLua.RegisterFunction("OnLoopMontageBlendOut", GARush.OnLoopMontageBlendOut)

function GARush:OnLoopMontageInterrupted()
    self:PlayEndMontage()
end
UE.DistributedDSLua.RegisterFunction("OnLoopMontageInterrupted", GARush.OnLoopMontageInterrupted)

function GARush:OnLoopMontageCancelled()
    self:K2_EndAbility()
end
UE.DistributedDSLua.RegisterFunction("OnLoopMontageCancelled", GARush.OnLoopMontageCancelled)
...
UE.DistributedDSLua.RegisterFunction("OnRushEndMontageCompleted", GARush.OnRushEndMontageCompleted)

function GARush:OnRushEndMontageInterrupted()
    G.log:debug(self.__TAG__, "OnRushEndMontageInterrupted")
    self:K2_EndAbility(true)
end
UE.DistributedDSLua.RegisterFunction("OnRushEndMontageInterrupted", GARush.OnRushEndMontageInterrupted)
...
UE.DistributedDSLua.RegisterCustomClass("GARush", GARush, GAPlayerBase)
