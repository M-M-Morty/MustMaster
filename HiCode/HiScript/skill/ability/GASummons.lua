...
---临时 这里做了角色隐藏，后续正式方案来了后需要overrider
function GASummons:OnBlendOut(...)
    G.log:error(self.__TAG__, "GASummons %s OnBlendOut, IsServer: %s.", UE.UKismetSystemLibrary.GetDisplayName(self), self:IsServer())
    local Actor = self.OwnerActor
    if Actor then
        ...
        if isPlayer then
            ...
            if Actor:GetName() ~= Player:GetName() then
                Player:SetActorHiddenInGame(false)
            end
        end
    end
    Super(GASummons).OnBlendOut(self)
end

function GASummons:OnCompleted()
    G.log:error(self.__TAG__, "GASummons %s OnCompleted, IsServer: %s.", UE.UKismetSystemLibrary.GetDisplayName(self), self:IsServer())
    Super(GASummons).OnCompleted(self)
end
