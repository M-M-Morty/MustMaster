...
local SWITCH_PLAYER_INVEHICLE = "SWITCH_PLAYER_INVEHICLE"
...
function PlayerController:SwitchPlayer(CharType, bPlayerDeadReason, bInSwitchQTE)
    G.log:info("devin", "PlayerController:SwitchPlayer")
    
    local NewPlayer = self:GetCharacterByCharType(CharType)
    
    if not bInSwitchQTE and not NewPlayer:IsDead() then
...
function PlayerController:ServerSwitchPlayer_RPC(CharType, ExtraInfo)
    ...
    local bPlayerInQTE = ExtraInfo.bInQTE
    if bPlayerDeadReason then
        self.NextSwitchPlayerTime = 0
    else
        if not bPlayerInQTE then
            local NowMs = G.GetNowTimestampMs()
            if self.NextSwitchPlayerTime ~= 0 and self.NextSwitchPlayerTime > NowMs then
                return
            end
    ...
    if OldPlayer then
        if OldPlayer.Vehicle ~= nil then
            self:Client_OnSwitchPlayerFail(CharType, SWITCH_PLAYER_INVEHICLE)
            return
        end
...
function PlayerController:GetItemManager()
    return self.PlayerState.ItemManager
end
