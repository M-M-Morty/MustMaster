...
function SkillComponent:ReceiveBeginPlay()
    ...
    if self.actor:IsClient() then
        ...
        if self.actor.PlayerState then
            local AbilityType = self.actor.CAreaAbilityComponent:GetAreaAbilityType()
...
function SkillComponent:EnableSkillCost(SkillID, bEnableCost, bReplicated)
   ...
   -- local GA = self:FindAbilityFromSkillID(SkillID)
    if GA then
...
decorator.message_receiver()
function SkillComponent:OnReceiveMessageAfterSwitchOut()
...
function SkillComponent:OnEndZeroGravity()
    if self.SkillDriver then
        ...
        self:SendMessage("OnBreakSKill", "falling stop montage")
...
function SkillComponent:__OnServerReady()
    ...
    G.log:debug(self.__TAG__, "OnServerReady init skill component")
...
function SkillComponent:OnMovementModeChanged(PrevMovementMode, NewMovementMode, PreCustomMode, NewCustomMode)
    ...
    if NewMovementMode ~= UE.EMovementMode.MOVE_Walking and NewMovementMode ~= UE.EMovementMode.MOVE_NavWalking then
        ...
        if not ASC:HasGameplayTag(self.OnGroundTag) and bOnGround then
...
function SkillComponent:ReceiveAreaAbility(ItemActor)
    ...
    if self.actor.CAreaAbilityComponent then
        if ItemActor.cAreaAbility then
            G.log:info("ys", "skillComponent : ReceiveAreaAbility.cAreaAbility = %s", Enum.E_AreaAbility.GetDisplayNameTextByValue(ItemActor.cAreaAbility))
            self.actor.CAreaAbilityComponent:AreaAbilityCopyEffect(ItemActor.cAreaAbility)
        end
    end
end
...
function SkillComponent:OpenAreaAbilityPanel()
    self.actor.CAreaAbilityComponent:Server_LoadAreaAbilityData()
    self:OpenAreaAbilityUsePanel()
    self:OpenCopyerPanel()
end

decorator.message_receiver()
function SkillComponent:OpenAreaAbilityUsePanel()
    local VMDef = require('CP0032305_GH.Script.viewmodel.vm_define')
    ...
    if GameState then
        local AbilityType = self.actor.CAreaAbilityComponent:GetAreaAbilityType()
        if not AbilityType or AbilityType == Enum.E_AreaAbility.None then
            return
        end
        if not self.actor.CAreaAbilityComponent.bEnableAreaAbility then -- 马杜克等禁用状态
            return
        end
        ...
        if InputComponent then
            ...
            local function LeftMouseClick(InputHandler, value)
                ...
                if value then
                    ...
                    local PlayerLocation = PlayerActor:GetSocketLocation(PlayerActor.CAreaAbilityComponent.AreaAbilityFlyAttachName)
                    ...
                    self.actor.CAreaAbilityComponent:Server_AreaAbilityUse(nil, AbilityType, StartLocation, self.AreaAbilityUseLocation)
                else
            ...
            local function RightMouseClick(InputHandler, value)
                ...
                if not self.actor.CAreaAbilityComponent then
                    return
                end

                local PlayerActor = self.actor
                local AbilityType = self.actor.CAreaAbilityComponent:GetAreaAbilityType()
                if AbilityType == Enum.E_AreaAbility.None then -- 没有获得能力，无法使用
                    return
                end
                HiAudioFunctionLibrary.PlayAKAudio("Scn_Skills_Light_Cast", self.actor)
                local StartLocation = self.actor:K2_GetActorLocation()
                self.actor.CAreaAbilityComponent:Server_AreaAbilityUse(self.actor, AbilityType, StartLocation, UE.FVector(0,0,0))
            end
        ...
        self.actor.CAreaAbilityComponent:AreaAbilityScanEffect(AbilityType)
        
        --AreaAbilityVM:OpenAreaAbilityPanel()
        self.actor.CAreaAbilityComponent.bInAreaAbility = true
...
function SkillComponent:CloseAreaAbilityPanel(bAreaAbilityVM_SetCanExist_)
    ...
    if GameState then
        ...
        self.actor.CAreaAbilityComponent:AreaAbilityScanEffectEnd()
        if self.SpecialOpenUI then
            self.SpecialOpenUI:CloseMyself()
        end
        GameState:PlayerStopAimingMode()
        self.AreaAbilityPanelType = nil
        self.actor.CAreaAbilityComponent.bInAreaAbility = false
...
function SkillComponent:EnableAreaAbility(bEnable)
    ...
    self.actor.CAreaAbilityComponent.bEnableAreaAbility = bEnable
    local AreaAbilityVM = ViewModelCollection:FindUniqueViewModel(VMDef.UniqueVMInfo.AreaAbilityVM.UniqueName)
    if AreaAbilityVM then
        if bEnable then
            local AbilityType = self.actor.CAreaAbilityComponent:GetAreaAbilityType()
            ...
            AreaAbilityVM:SetAreaAbilityType(Enum.E_AreaAbility.None)
        end
    end
end
    ...
    if GameState then
        ...
        if InputComponent then
            ...
            local function LeftMouseClick(InputHandler, value)
                if value then
                    local AbilityType = self.actor.CAreaAbilityComponent:GetAreaAbilityType()
                    if AbilityType and AbilityType ~= Enum.E_AreaAbility.None then -- TODO(dougzhang): 当前只能获取一个照明能力
                        return
                    end
                    if self.AreaAbilityDetectActor then
                        if self.AreaAbilityDetectActor.eAreaAbility and self.AreaAbilityDetectActor.eAreaAbility ~= Enum.E_AreaAbility.None then
                            self.bAreaAbilityVM_SetCanExist = true
                            AreaAbilityVM:EnterReplicatorFocusState()
                            AreaAbilityVM:SetCanExist(false)
                            local Location = self.AreaAbilityDetectActor:K2_GetActorLocation()
                            G.log:info("ys", "self.AreaAbilityDetectActor.eAreaAbility = %s",self.AreaAbilityDetectActor.eAreaAbility)
                            self.actor.CAreaAbilityComponent:Server_AreaAbilityCopy(Location, self.AreaAbilityDetectActor.eAreaAbility)
                        end
                    end
                else
                    AreaAbilityVM:EnterReplicatorUnFocusState()
...
function SkillComponent:CloseCopyerPanel(bAreaAbilityVM_SetCanExist_)
    ...
    if GameState then
        ...
        OutlinerUtils:SetOverlapActorOutline(self.AreaAbilityDetectActor, false)
        if self.SpecialOpenUI then
            self.SpecialOpenUI:CloseMyself()
        end
        GameState:PlayerStopAimingMode()
...
------------ Switch Start -------------
decorator.message_receiver()
function SkillComponent:OnReceiveMessageBeforeSwitchOut()
    local AnimateAbilityHandle = self.AbilitySystemComponent:GetActiveAnimatingAbilityHandle()
    if not AnimateAbilityHandle then
        return
    end
    
    local AbilityInstance = self.AbilitySystemComponent:GetAbilityInstanceFromAbilityHandle(AnimateAbilityHandle)
    if not AbilityInstance then
        return
    end
    
    AbilityInstance:HandleSwitchOut()
end

decorator.message_receiver()
function SkillComponent:SwitchFadingOutFinished()
    local AnimateAbilityHandle = self.AbilitySystemComponent:GetActiveAnimatingAbilityHandle()
    if AnimateAbilityHandle then
        self.AbilitySystemComponent:BP_CancelAbilityHandle(AnimateAbilityHandle)
    end
end
