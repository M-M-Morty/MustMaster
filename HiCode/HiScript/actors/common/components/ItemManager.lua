...
---@param TabIndex integer
---@return void
function BP_ItemManager:Client_TabRedDot_RPC(TabIndex, bHasRedDot)
    if UE.UKismetSystemLibrary.IsServer(self) then
        return
    end
    if not ItemUtil.IsBagTabExist(TabIndex) then
    ...
    SetTabRedDot(self, TabIndex, bHasRedDot)
    OnTabNewRedDotCallback(self, TabIndex, bHasRedDot)
end
...
local function AddItemByExcelID(self, ExcelID, Count, AddItems, postCreate)
...
local function AddStackItems(self, ExcelID, Count, StackLimit, AddItems)
    local ItemConfig = ItemUtil.GetItemConfigByExcelID(ExcelID)
    local ExistCount = self:GetItemCountByExcelID(ExcelID)
    local pf_setup_new = nil
    if ItemConfig.appear_rule == ItemBaseTable.New and ExistCount < 1 then
        pf_setup_new = function(Item)
            Item.bNew=true
        end
    end
    ...
    for i = 1, Count/StackLimit + 1 do
        if RemainCount > 0 then
            ...
            AddItemByExcelID(self, ExcelID, StackCount, AddItems, pf_setup_new)
        end
    end
end
...
function BP_ItemManager:AddItemByExcelID(ExcelID, Count)
    ...
    if not bAutoUse and ItemConfig.appear_rule == ItemBaseTable.Redpoint then
        ...
        self:Client_TabRedDot(TabIndex, true)
    end
...
function BP_ItemManager:ReduceItemByExcelID(ExcelID, Count)
    ...
    local TabIdx = ItemUtil.GetItemTabIndex(ExcelID)
    Items = self:GetItemsByTabID(TabIdx)
    local RedDot = false
    for i , item in ipairs(Items) do
        if item.bNew or item.bUsed then
            RedDot = true
            break
        end
    end
    SetTabRedDot(self, TabIdx, RedDot)
    self:Client_TabRedDot(TabIdx, RedDot)
