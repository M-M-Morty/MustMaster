...
function MonsterAppearance:OnDead()
    if not self.AnimInstance.IsDead then
        ...
        if self.DefaultDeadAnimation then
            G.log:debug(self.__TAG__, "OnDead after finish play animation")
            ...
            -- 经过多次测试，的确就有下面三个分支都没有走到的情况，montage没有开始执行，被某个动作打断了，但是没有触发interrupt
            -- PlayMontageCallbackProxy.OnInterrupted:Add(self.actor, DeadCallback)
            -- PlayMontageCallbackProxy.OnBlendOut:Add(self.actor, DeadCallback)
            -- PlayMontageCallbackProxy.OnCompleted:Add(self.actor, DeadCallback)

            utils.DoDelay(self.actor, self.DefaultDeadAnimation:GetPlayLength(), function ( ... )
                if self.actor then
                    self.actor:K2_DestroyActor()
                end
            end)
