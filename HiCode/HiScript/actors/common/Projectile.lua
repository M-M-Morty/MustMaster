...
function Projectile:OnBeRebound(SourceActor, TargetActor)
    self.CharCamp = SourceActor.CharCamp
    self.Spec.MoveTyp = Enum.Enum_MoveType.MoveTypeFixed
    self.SourceActor = SourceActor
    self.SkillTarget = TargetActor
    local ReboundDir
    if self.SkillTarget then
        --G.log:error("yb", "OnReboundTarget:%s", G.GetObjectName(self.SkillTarget))
        --UE.UKismetSystemLibrary.DrawDebugLine(self.SkillTarget, self.SkillTarget:K2_GetActorLocation(), self.SkillTarget:K2_GetActorLocation() + UE.FVector(0,0, 1) * 200, UE.FLinearColor(1, 0, 0), 20, 5)
        ReboundDir = self.SkillTarget:K2_GetActorLocation() - self.SourceActor:K2_GetActorLocation()
        ReboundDir = UE.UKismetMathLibrary.Conv_VectorToRotator(ReboundDir)
    else
        ReboundDir = self.SourceActor:K2_GetActorRotation()
    end
    --local Dir = ReboundDir:GetForwardVector()
    --UE.UKismetSystemLibrary.DrawDebugLine(self, self:K2_GetActorLocation(), self:K2_GetActorLocation() + Dir * 200, UE.FLinearColor(1, 0, 1), 20, 5)
    -- 子弹有限弹给技能当前施法目标，子弹则按照当前角色朝向进行飞行
    --self:OnBeExtremeWithStand(self.SourceActor)
    self:K2_SetActorRotation(ReboundDir , true)
    self:SendMessage("InitCalcForHits", self.SourceActor, self,
        self.Spec, self.KnockInfo, self.GameplayEffectsHandle, self.HitSceneTargetConfig, nil, self.SelfGameplayEffectsHandle)
...
function Projectile:MoveSourceForward(DeltaSeconds)
    ...
    --G.log:info(self.__TAG__, "Projectile:MoveSourceForward %s %s",G.GetObjectName(self), tostring(self:K2_GetRootComponent():GetForwardVector()))
...
function Projectile:ExecCalcInstant(bOnDestroy, HitPoint)
    ...
    if self.LastSweepPos then
        ...
        --G.log:info("YB", "Projectile:ExecCalcInstant HITS:%d", Hits:Num())
...
function Projectile:ExecCalc(Hits, bOnDestroy)
    ...
    for Index = 1, Hits:Length() do
        repeat
            ...
            --检查目标是否免疫子弹技能
            if actor.BP_ImmuneProjectileComponent and actor.BP_ImmuneProjectileComponent:CheckImmuneProjectile() then
                break
            end
