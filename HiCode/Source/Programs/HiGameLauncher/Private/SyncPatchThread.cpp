...
public:
	...
	//needDownloadSize, LocalDiffSize
	TFunction<void(int64, int64)> CallbackFunc = nullptr;
	...
	static void OnNeedSyncInfo_V2(ISyncInfoListener* listener, const TNeedSyncInfos* needSyncInfo)
	{
		if (listener)
		{
			...
			{
				CallbackWrapper->CallbackFunc(needSyncInfo->needSyncSumSize, needSyncInfo->localDiffDataSize);
			}
		}
	}
};
...
public:
	...
	FReadDataStream(const FString& InFileName, bool InAllowWrite = false)
	{
		FileName = InFileName;
		read = FReadDataStream::OnRead_V2;
		FileHandle = FPlatformFileManager::Get().GetPlatformFile().OpenRead(*FileName, InAllowWrite);
	...
	IFileHandle* GetFileHandle() const
	{
		return FileHandle;
	}
...
struct FWriteNewDataStream : public hpatch_TStreamOutput
{
public:

	FWriteNewDataStream(const FString& InFileName, bool bAppendMode = false, bool bAllowRead = false)
	{
		FileName = InFileName;
		FileHandle = FPlatformFileManager::Get().GetPlatformFile().OpenWrite(*FileName, bAppendMode, bAllowRead);
		write = FWriteNewDataStream::OnWrite_V2;
		...
		if (bAllowRead)
		{
			read_writed = FWriteNewDataStream::OnRead_V2;
	...
	void OpenFileHandle(bool bAppendMode, bool bAllowRead)
	{
		if (FileHandle == nullptr)
		{
			FileHandle = FPlatformFileManager::Get().GetPlatformFile().OpenWrite(*FileName, bAppendMode, bAllowRead);
	...
	{
		int64 RetValue = hpatch_FALSE;
		const FWriteNewDataStream* DataStream = static_cast<const FWriteNewDataStream*>(stream);
		if (DataStream->FileHandle != nullptr)
		{
	...
	//unsigned char* out_data,unsigned char* out_data_end);
	static hpatch_BOOL OnRead_V2(const hpatch_TStreamOutput* stream,
		...
		TByte* out_data, TByte* out_data_end)
	{
		...
		{
			DataStream->FileHandle->Seek(readFromPos);
			if ((int64)readFromPos < DataStream->FileHandle->Size() && ((int64)readFromPos + ReadSize) <= DataStream->FileHandle->Size())
			{
				bool bRet = DataStream->FileHandle->Read(out_data, out_data_end - out_data);
				if (bRet)
				{
...
TSyncClient_resultType FSyncPatchThread::AnalyzePatchInfo(FHiGameVersionInfo& InPatchInfo)
{
	...
	for (int32 Idx = 0; Idx < InPatchInfo.FileList.Num(); ++Idx)
	{
		...
		{
			int64 TempNeedDiffSize = -1;
			int64 TempLocalDiffSize = -1;
			...
			FMemory::Memzero(&SyncInfoListener, sizeof(SyncInfoListener));
			{
				...
				SyncInfoListener.CallbackFunc = [this, &TempNeedDiffSize, &TempLocalDiffSize, PatchInfo = &InPatchInfo.FileList[Idx], Idx](int64 InNeedDownloadSize, int64 InLocalDiffSize) {
					TempNeedDiffSize = InNeedDownloadSize;
					TempLocalDiffSize = InLocalDiffSize;
					PatchInfo->PatchSize = InNeedDownloadSize;
					PatchInfo->DownloadedSize = TempLocalDiffSize;
					};
			}
			...
			int64 ExistDiffFileSize = FPlatformFileManager::Get().GetPlatformFile().FileSize(*DiffFileName);
			FWriteNewDataStream out_diffData(DiffFileName, true, ExistDiffFileSize > 0);

			FReadDataStream diffContinueData(DiffFileName, true);

			FReadDataStream OldDataStream(FileName);
			...
			RetValue = sync_local_diff(&SyncInfoListener, &ReadSyncDataListener, &OldDataStream, &newSyncInfo, &out_diffData, kSyncDiff_info, ExistDiffFileSize > 0 ? &diffContinueData : nullptr, 4);

			TNewDataSyncInfo_close(&newSyncInfo);

			//TODO:如果差异更新超过一定比率就下载完整文件
			float DiffPercent = 100.f * (TempNeedDiffSize - TempLocalDiffSize) / InPatchInfo.FileList[Idx].Size;
			...
			UE_LOG(LogTemp, Log, TEXT("NeedDiffSize %lld, LocalDiffSize %lld"), TempNeedDiffSize, TempLocalDiffSize);
#endif
		}

		if (RetValue != kSyncClient_ok)
		{
			UE_LOG(LogTemp, Warning, TEXT("sync_local_diff(kSyncDiff_info) error, %d"), RetValue);
			break;
		}
	}
...
TSyncClient_resultType FSyncPatchThread::DownloadPatch(FHiGameVersionInfo InPatchInfo)
{
	...
	for (int32 Idx = 0; Idx < InPatchInfo.FileList.Num(); ++Idx)
	{
		...
		bool bDownloadBegin = false;

		//下载进度
		FDownloadProgressCallback ProgressCallback = [this, &bDownloadBegin, PatchInfo = &InPatchInfo.FileList[Idx]](int64 InCurrentSize, int64 InTotalSize)
			{
		...
		//sync patch模式（差异更新）
		{
			...
			//读取旧文件进度
			OldDataStream.ProgressCallback = [this, &bDownloadBegin, PatchInfo = InPatchInfo.FileList[Idx]](int64 InReadPosition)
				{
					if (!bDownloadBegin && EventList.IsEmpty())
					{
						FUpdaterEventData NextEvent;
						NextEvent.State = WorkingState;
						NextEvent.CurrentSize = InReadPosition;
			...
			FWriteNewDataStream out_diffData(DiffFileName, true, true);

			FReadDataStream diffContinueData(DiffFileName, true);

			RetValue = sync_local_diff(&SyncInfoListener, &ReadSyncDataListener, &OldDataStream, &newSyncInfo, &out_diffData, kSyncDiff_default, &diffContinueData, 4);

			//可能是之前下载到一半的diff无效了（文件损坏或CDN上文件变化了）
			if (RetValue != kSyncClient_ok)
			{
				InPatchInfo.FileList[Idx].DownloadedSize = 0;

				out_diffData.CloseFileHandle();
				diffContinueData.CloseFileHandle();

				FPlatformFileManager::Get().GetPlatformFile().DeleteFile(*DiffFileName);

				out_diffData.OpenFileHandle(false, false);

				RetValue = sync_local_diff(&SyncInfoListener, &ReadSyncDataListener, &OldDataStream, &newSyncInfo, &out_diffData, kSyncDiff_default, nullptr, 4);
				UE_LOG(LogTemp, Warning, TEXT("sync_local_diff(kSyncDiff_default) error, try delete diff file %s"), *DiffFileName);
			}
		...
		if (RetValue != kSyncClient_ok)
		{
			UE_LOG(LogTemp, Warning, TEXT("sync_local_diff(kSyncDiff_default) error, %d"), RetValue);
			break;
		}
	}
...
TSyncClient_resultType FSyncPatchThread::ApplyPatch(FHiGameVersionInfo InPatchInfo)
{
	...
	for (int32 Idx = 0; Idx < InPatchInfo.FileList.Num(); ++Idx)
	{
		...
		if (!FullyDownloadFileList.Contains(*InPatchInfo.FileList[Idx].Name))
		{
			...
			hpatch_TFileStreamInput_open(&in_oldData, TCHAR_TO_UTF8(*FileName));

			TNewDataSyncInfo newSyncInfo;
			sync_private::TNewDataSyncInfo_init(&newSyncInfo);
			TNewDataSyncInfo_open_by_file(&newSyncInfo, TCHAR_TO_UTF8(*NewFileIndex), &SyncInfoListener);

			FWriteNewDataStream NewFileStream(PatchedFileName, false, false);
			NewFileStream.streamImport = &NewFileStream;
			NewFileStream.streamSize = 0;
			//NewFileStream.write = &FWriteNewDataStream::OnWrite_V2;
...
TSyncClient_resultType FSyncPatchThread::VerifyAndMoveFile(FHiGameVersionInfo InPatchInfo)
{
	...
	if (RetValue == kSyncClient_ok && InPatchInfo.FileList.Num() > 0)
	{
