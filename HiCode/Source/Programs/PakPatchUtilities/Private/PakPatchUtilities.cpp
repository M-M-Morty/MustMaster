...
bool ListFilesInPak(const TCHAR* InPakFilename, int64 SizeFilter, bool bIncludeDeleted, const FString& CSVFilename, bool bExtractToMountPoint, const FKeyChain& InKeyChain, bool bAppendFile)
{
	...
	if (PakFile.IsValid())
	{
		...
		//HiGame Begin
		TArray<int64> EntrySizeList;
		EntrySizeList.SetNum(FileCount);
		//HiGame End
		...
		for (auto It : Records)
		{
			...
			//HiGame Begin
			FArchive& Reader = PakFile.GetSharedReader(LowerLevelPlatformFile).GetArchive();
			Reader.Seek(It.Info().Offset);
			FPakEntry SerializedEntry;
			SerializedEntry.Serialize(Reader, PakFile.GetInfo().Version);
			EntrySizeList[EntryIndex - 1] = SerializedEntry.GetSerializedSize(PakFile.GetInfo().Version);
			//HiGame End
		}
		...
		if (CSVFilename.Len() > 0)
		{
			...
			if (!bAppendFile)
			{
				...
				OutputArchive->Logf(TEXT("Filename, Offset, Size, Hash, Deleted, Compressed, CompressionMethod, EntrySize"));
			}
			EntryIndex = 0;
			for (auto It : Records)
			{
				...
				*/
				...
				OutputArchive->Logf(
					TEXT("%s%s, %lld, %lld, %s, %s, %s, %d, %lld"),
					*MountPoint, It.TryGetFilename() ? **It.TryGetFilename() : TEXT("<FileNamesNotLoaded>"),
					Entry.Offset, Entry.Size,
					* BytesToHex(Hash, sizeof(FPakEntry::Hash)),
					Entry.IsDeleteRecord() ? TEXT("true") : TEXT("false"),
					bWasCompressed ? TEXT("true") : TEXT("false"),
					Entry.CompressionMethodIndex, EntrySizeList[EntryIndex - 1]);
 ...
 */
bool ExecutePakPatch(const TCHAR* CmdLine)
{
	{
		FString IoStoreArg;
	...
	if (FParse::Value(CmdLine, TEXT("-Batch="), BatchFileName))
	{
		...
		ParallelFor(Commands.Num(), [&Commands, &Result](int32 Idx) { if (!ExecutePakPatch(*Commands[Idx])) { Result = false; } });
		return Result;
	}
...
int32 FPakPatchUtilities::DiffPak(const FString& InOldPak, const FString& InNewPak, const FString& InPatchPak)
{

	IPlatformFile* LowerLevelPlatformFile = &FPlatformFileManager::Get().GetPlatformFile();
	TRefCountPtr<FPakFile> OldPakFile = new FPakFile(LowerLevelPlatformFile, *InOldPak, false);
	TRefCountPtr<FPakFile> NewPakFile = new FPakFile(LowerLevelPlatformFile, *InNewPak, false);

	if (!OldPakFile->IsValid() || !NewPakFile->IsValid())
	{
		return false;
	}

	FPakInfo OldPakInfo = OldPakFile->GetInfo();
	FPakInfo NewPakInfo = NewPakFile->GetInfo();

	FString OldMountPoint = OldPakFile->GetMountPoint();
	FString NewMountPoint = NewPakFile->GetMountPoint();

	TArray<FPakEntryPair> OldPakIndex;
	TArray<FPakEntryPair> NewPakIndex;

	TSet<FString> OldFileNameList;
	TSet<FString> NewFileNameList;

	for (FPakFile::FPakEntryIterator It(*OldPakFile, true); It; ++It)
	{
		FPakEntry Entry = It.Info();
		OldPakFile->ReadHashFromPayload(Entry, Entry.Hash);
		FString EntryFileName = OldMountPoint / *It.TryGetFilename();
		FPakEntryPair NewEntryPair{ EntryFileName, Entry };
		OldPakIndex.Add(NewEntryPair);
		OldFileNameList.Add(EntryFileName);
	}

	for (FPakFile::FPakEntryIterator It(*NewPakFile, true); It; ++It)
	{
		FPakEntry Entry = It.Info();
		NewPakFile->ReadHashFromPayload(Entry, Entry.Hash);
		FString EntryFileName = NewMountPoint / *It.TryGetFilename();
		FPakEntryPair NewEntryPair{ EntryFileName, Entry };
		NewPakIndex.Add(NewEntryPair);
		NewFileNameList.Add(EntryFileName);
	}
	...
	TArray<FString> AddedFileList = NewFileNameList.Difference(OldFileNameList).Array();
	TArray<FString> RemovedFileList = OldFileNameList.Difference(NewFileNameList).Array();
	TArray<FString> SameFileList = OldFileNameList.Intersect(NewFileNameList).Array();

	//删除的信息需要从旧pak获取（文件名）
	TArray<FPakEntryPair> RefOldPakIndex;

	//增删改的（新pak中某个文件本身可能是标记删除的）
	TArray<FPakEntryPair> RefNewPakIndex;

	TArray<FPakInputPair> FinalFileNameList;

	//此处循环可以优化，先将就
	for (FString SameFile : SameFileList)
	{
		FPakEntry EntryInOld;
		bool bFoundInOld = false;
		for (FPakEntryPair Pair : OldPakIndex)
		{
			if (SameFile == Pair.Filename)
			{
				EntryInOld = Pair.Info;
				bFoundInOld = true;
				break;
			}
		}
		...
		FPakEntry EntryInNew;
		bool bFoundInNew = false;
		for (FPakEntryPair Pair : NewPakIndex)
		{
			if (SameFile == Pair.Filename)
			{
				EntryInNew = Pair.Info;
				bFoundInNew = true;
				break;
			}
		}
		...
		if (EntryInOld.IsDeleteRecord() == EntryInNew.IsDeleteRecord()
			&& FMemory::Memcmp(EntryInOld.Hash, EntryInNew.Hash, 20) == 0)
		{
			//same content
			continue;
		}

		RefNewPakIndex.Add(FPakEntryPair{ SameFile, EntryInNew });

		FinalFileNameList.Add(FPakInputPair(SameFile, SameFile));
	}

	for (FString FileName : AddedFileList)
	{
		FPakEntry EntryInNew;

		for (FPakEntryPair NewPair : NewPakIndex)
		{
			if (FileName == NewPair.Filename)
			{
				EntryInNew = NewPair.Info;
				RefNewPakIndex.Add(FPakEntryPair{ FileName, EntryInNew });
				FinalFileNameList.Add(FPakInputPair(FileName, FileName));
				break;
			}
		}
	}

	for (FString FileName : RemovedFileList)
	{
		FPakEntry EntryInOld;

		for (FPakEntryPair OldPair : OldPakIndex)
		{
			if (FileName == OldPair.Filename)
			{
				EntryInOld = OldPair.Info;
				RefOldPakIndex.Add(FPakEntryPair{ FileName, EntryInOld });
				FinalFileNameList.Add(FPakInputPair(FileName, FileName));
				break;
			}
		}
	...
	FString FinalMountPoint = GetCommonRootPath(FinalFileNameList);
	int32 MountPointLen = FinalMountPoint.Len();

	FPakInfo PatchPakInfo;

	//更新包只会引用新版pak中的压缩数据内容，所以只需要记录新版pak的压缩方法即可
	//注意，在客户端合并时需要处理旧版pak压缩方法和补丁包中压缩方法不一样的情况，
	//需要在合并时重新调整patch包内文件的CompressIndex
	//可以考虑禁用zlib，全部使用oodle，问题就简单多了
	PatchPakInfo.CompressionMethods = NewPakInfo.CompressionMethods;

	FArchive* PakWriter = IFileManager::Get().CreateFileWriter(*InPatchPak, EFileWrite::FILEWRITE_Append);
	FArchive* NewPakReader = IFileManager::Get().CreateFileReader(*InNewPak);

	if (PakWriter == nullptr || NewPakReader == nullptr)
	{
		return -1;
	}

	TArray<FPakEntryPair> PatchPakIndex;

	int64 DataPosition = 0;
	for (FPakEntryPair NewEntry : RefNewPakIndex)
	{
		NewEntry.Filename = NewEntry.Filename.Mid(MountPointLen);

		if (!NewEntry.Info.IsDeleteRecord())
		{
			//从patchpak读取数据段
			TArray<uint8> NewFileData;
			{
				int64 EntrySize = NewEntry.Info.GetSerializedSize(NewPakInfo.Version);
				NewPakReader->Seek(EntrySize + NewEntry.Info.Offset);
				int64 NewFileSize = NewEntry.Info.Size;
				NewFileData.SetNum((int32)NewFileSize);
				NewPakReader->Serialize(NewFileData.GetData(), NewFileSize);
			}

			NewEntry.Info.Offset = DataPosition;

			PakWriter->Seek(DataPosition);

			int64 EntrySize = NewEntry.Info.GetSerializedSize(PatchPakInfo.Version);
			NewEntry.Info.Serialize(*PakWriter, PatchPakInfo.Version);
			PakWriter->Serialize(NewFileData.GetData(), NewFileData.Num());
			DataPosition += (EntrySize + NewFileData.Num());
		}
		else
		{
			NewEntry.Info.Offset = -1;
		}

		PatchPakIndex.Add(NewEntry);

	}

	NewPakReader = nullptr;

	PatchPakInfo.IndexOffset = DataPosition;

	for (FPakEntryPair OldEntry : RefOldPakIndex)
	{
		OldEntry.Filename = OldEntry.Filename.Mid(MountPointLen);
		OldEntry.Info.Offset = -1;
		OldEntry.Info.SetDeleteRecord(true);
		PatchPakIndex.Add(OldEntry);
	}

	//写入Index
	FPakFooterInfo Footer(*InPatchPak, FinalMountPoint, PatchPakInfo, PatchPakIndex);
	PakWriter->Seek(PatchPakInfo.IndexOffset);
	WritePakFooter(*PakWriter, Footer);
	PakWriter->Flush();
	PakWriter->Close();
...
int32 FPakPatchUtilities::PatchPak(const FString& InSourcePak, const FString& InPatchPak)
{
	static const FString RPG_PAK_DELETE_FLAG = FString("RPG_PAK_DELETE_FLAG_20240514");

	IPlatformFile* LowerLevelPlatformFile = &FPlatformFileManager::Get().GetPlatformFile();
	TRefCountPtr<FPakFile> SourcePakFile = new FPakFile(LowerLevelPlatformFile, *InSourcePak, false);
	TRefCountPtr<FPakFile> PatchPakFile = new FPakFile(LowerLevelPlatformFile, *InPatchPak, false);

	if (!SourcePakFile->IsValid() || !PatchPakFile->IsValid())
	{
		return false;
	}

	FPakInfo PakInfo = SourcePakFile->GetInfo();
	FPakInfo PakPatchInfo = PatchPakFile->GetInfo();

	//检测旧pak和补丁pak内的压缩方法是否不一致，如果不一致则在复制patch内文件到原pak过程中需要调整CompressionMethodIndex
	bool bSameCompressionMethods = true;
	if (PakInfo.CompressionMethods.Num() != PakPatchInfo.CompressionMethods.Num())
	{
		bSameCompressionMethods = false;
	}

	if (bSameCompressionMethods)
	{
		for (int32 idx = 0; idx < PakInfo.CompressionMethods.Num(); ++idx)
		{
			if (PakInfo.CompressionMethods[idx] != PakPatchInfo.CompressionMethods[idx])
			{
				bSameCompressionMethods = false;
				break;
			}
	...
	int64 SourcePakSize = SourcePakFile->TotalSize();
	int64 PakPatchTotalSize = PatchPakFile->TotalSize();

	FString MountPoint = SourcePakFile->GetMountPoint();
	FString PatchMountPoint = PatchPakFile->GetMountPoint();

	TArray<FPakEntryPair> SourcePakIndex;
	TArray<FPakEntryPair> PatchPakIndex;

	TArray<FPakInputPair> FinalFileNameList;

	for (FPakFile::FPakEntryIterator It(*SourcePakFile, true); It; ++It)
	{
		FPakEntry Entry = It.Info();
		SourcePakFile->ReadHashFromPayload(Entry, Entry.Hash);
		FString EntryFileName = MountPoint / *It.TryGetFilename();
		FPakEntryPair NewEntryPair{ EntryFileName, Entry };
		SourcePakIndex.Add(NewEntryPair);

		FinalFileNameList.Add(FPakInputPair(EntryFileName, EntryFileName));
	}

	for (FPakFile::FPakEntryIterator It(*PatchPakFile, true); It; ++It)
	{
		FPakEntry Entry = It.Info();
		PatchPakFile->ReadHashFromPayload(Entry, Entry.Hash);
		FString EntryFileName = PatchMountPoint / *It.TryGetFilename();
		FPakEntryPair NewEntryPair{ EntryFileName, Entry };
		PatchPakIndex.Add(NewEntryPair);
	...
	//调整文件虚拟路径（挂载点可能变化了）
	int32 MountPointLen = FinalMountPoint.Len();
	for (FPakEntryPair& It : SourcePakIndex)
	{
		It.Filename = It.Filename.Mid(MountPointLen);
	}

	//下面有读取文件名字判断新旧pak中是否都存在，上面改了原pak中文件名，此处把patch中也按照新挂载点刷新下
	for (FPakEntryPair& It : PatchPakIndex)
	{
		It.Filename = It.Filename.Mid(MountPointLen);
	}

	//释放文件句柄，否则后续无法写入文件
	SourcePakFile = nullptr;
	PatchPakFile = nullptr;

	FArchive* PakWriter = IFileManager::Get().CreateFileWriter(*InSourcePak, EFileWrite::FILEWRITE_Append);
	FArchive* PatchPakReader = IFileManager::Get().CreateFileReader(*InPatchPak);

	if (PakWriter == nullptr || PatchPakReader == nullptr)
	{
		return -1;
	}

	//记录时间，用于定时保存index
	double LastTimeSerializeIndex = FPlatformTime::Seconds();

	//逐个遍历补丁包中的文件，并在基准pak中查找同名文件位置（优先），或废弃文件位置（有文件标记删除了）
	//如果该空间足以容纳新文件，则复制进去，并修改index；如果空间不够则在基准pak末尾追加

	for (int32 idx = 0; idx < PatchPakIndex.Num(); ++idx)
	{
		FPakEntryPair PatchItem = PatchPakIndex[idx];

		int32 PatchFileInSourcePakIndex = -1;
		FString PatchItemFileName = PatchItem.Filename;
		int64 PatchItemSize = PatchItem.Info.Size;

		for (int32 SourceItemIdx = 0; SourceItemIdx < SourcePakIndex.Num(); ++SourceItemIdx)
		{
			FPakEntryPair& BaseItem = SourcePakIndex[SourceItemIdx];
			if (PatchItemFileName != BaseItem.Filename && !PatchItemFileName.EndsWith(RPG_PAK_DELETE_FLAG))
			{
				continue;
			}

			int64 OldFileSize = BaseItem.Info.GetSerializedSize(PakInfo.Version) + BaseItem.Info.Size;
			int64 PatchFileSize = PatchItem.Info.GetSerializedSize(PakPatchInfo.Version) + PatchItem.Info.Size;

			//老版本文件的空间足够容纳新版文件时才能复用旧空间
			if (OldFileSize >= PatchFileSize)
			{
				PatchFileInSourcePakIndex = SourceItemIdx;
				break;
			}
			else
			{
				//空间不够，就需要追加到末尾;把原来位置标记废弃
				//此处修改还没有序列化，直到序列化完数据段，才会序列化index，
				//所以不会出现索引先于数据段修改（写入数据中断时先写入的index无效）
				BaseItem.Filename = FinalMountPoint / RPG_PAK_DELETE_FLAG;
				BaseItem.Info.SetDeleteRecord(true);
			}
		}

		//计算新文件项待写入位置，以及索引信息在数组中的位置
		int64 InsertPayloadPosition = -1;
		int32 InsertItemIndex = -1;
		if (PatchFileInSourcePakIndex >= 0)
		{
			InsertItemIndex = PatchFileInSourcePakIndex;
			InsertPayloadPosition = SourcePakIndex[PatchFileInSourcePakIndex].Info.Offset;
		}
		else
		{
			InsertPayloadPosition = PakInfo.IndexOffset;
			InsertItemIndex = SourcePakIndex.Num();
			//追加情况下需要计算对齐位置
			//TODO:计算PaddingSize
			PakInfo.IndexOffset += PatchItem.Info.GetSerializedSize(PakPatchInfo.Version) + PatchItemSize;
			SourcePakIndex.AddZeroed(1);
		}

		SourcePakIndex[InsertItemIndex] = PatchItem;

		//use base pak offset.
		SourcePakIndex[InsertItemIndex].Info.Offset = InsertPayloadPosition;

		//写入entry到basepak
		PakWriter->Seek(InsertPayloadPosition);
		SourcePakIndex[InsertItemIndex].Info.Serialize(*PakWriter, FPakInfo::PakFile_Version_Latest);
		int64 PayloadOffset = PakWriter->Tell();
		...
		TArray<uint8> NewFileData;
		{
			int64 EntrySize = PatchItem.Info.GetSerializedSize(PakPatchInfo.Version);
			PatchPakReader->Seek(EntrySize + PatchItem.Info.Offset);
			int64 NewFileSize = PatchItem.Info.Size;
			NewFileData.SetNum((int32)NewFileSize);
			PatchPakReader->Serialize(NewFileData.GetData(), NewFileSize);
		}
		...
		//每隔一段时间写入一次Index，尽可能规避意外终止导致basepak损坏（如断电，杀进程等）
		double NowTime = FPlatformTime::Seconds();
		if ((NowTime - LastTimeSerializeIndex > 5.f) || (idx + 1 == PatchPakIndex.Num()))
		{
			//写入Index
			FPakFooterInfo Footer(*InSourcePak, FinalMountPoint, PakInfo, SourcePakIndex);
			PakWriter->Seek(PakInfo.IndexOffset);
			WritePakFooter(*PakWriter, Footer);
			PakWriter->Flush();
			LastTimeSerializeIndex = NowTime;
		}
	}
...
TArray<FPakEntryPair> FPakPatchUtilities::ParsePakIndexFromCSV(const FString& InCsvFile)
{
	TArray<FPakEntryPair> NewPakIndex;

	TArray<FString> PakIndexData;

	FFileHelper::LoadFileToStringArray(PakIndexData, *InCsvFile);

	if (PakIndexData.Num() < 2)
	{
		return NewPakIndex;
	}

	FString DataStructLine = PakIndexData[0];
	FString DefaultDataStructStr = "Filename, Offset, Size, Hash, Deleted, Compressed, CompressionMethod, EntrySize";

	if (!DataStructLine.StartsWith(DefaultDataStructStr))
	{
		return NewPakIndex;
	}
	
	NewPakIndex.SetNum(PakIndexData.Num() - 1);

	for (int32 idx = 1; idx < PakIndexData.Num(); ++idx)
	{
		FString FileInfoLine = PakIndexData[idx];
		TArray<FString> FileInfos;
		FileInfoLine.ParseIntoArray(FileInfos, TEXT(", "), false);

		FPakEntryPair FileIndex;
		...
		FileIndex.Filename = FileInfos[0];
		FileIndex.Info.Offset = FCString::Atoi64(*FileInfos[1]);
		FileIndex.Info.UncompressedSize = FCString::Atoi64(*FileInfos[2]);

		HexToBytes(FileInfos[3], FileIndex.Info.Hash);
		FString HashHex = BytesToHex(FileIndex.Info.Hash, sizeof(FileIndex.Info.Hash));
		check(HashHex == FileInfos[3]);

		bool bDelete = FCString::ToBool(*FileInfos[4]);
		FileIndex.Info.SetDeleteRecord(bDelete);
		FileIndex.Info.Size = FCString::Atoi64(*FileInfos[5]);
		FileIndex.Info.CompressionMethodIndex = FCString::Atoi(*FileInfos[6]);

		NewPakIndex[idx - 1] = FileIndex;
	}

	return NewPakIndex;
}

int32 FPakPatchUtilities::DiffPakByCSV(const FString& InOldPak, const FString& InNewPakCSV)
{
	TArray<FPakEntryPair> NewPakIndex = ParsePakIndexFromCSV(InNewPakCSV);


	IPlatformFile* LowerLevelPlatformFile = &FPlatformFileManager::Get().GetPlatformFile();
	TRefCountPtr<FPakFile> OldPakFile = new FPakFile(LowerLevelPlatformFile, *InOldPak, false);

	if (!OldPakFile->IsValid() || NewPakIndex.IsEmpty())
	{
		return false;
	}
	...
	for (FPakEntryPair It : NewPakIndex)
	{
	...
	//可以考虑禁用zlib，全部使用oodle，问题就简单多了
	//PatchPakInfo.CompressionMethods = NewPakInfo.CompressionMethods;
	PatchPakInfo.CompressionMethods = OldPakInfo.CompressionMethods;
