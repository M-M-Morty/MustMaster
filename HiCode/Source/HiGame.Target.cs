...
public class HiGameTarget : TargetRules
{
	public HiGameTarget( TargetInfo Target) : base(Target)
	{
		bForceEnableExceptions = true;
		Type = TargetType.Game;
		DefaultBuildSettings = BuildSettingsVersion.V2;
		ProjectDefinitions.Add("HAVE_IRPC_IMPL=1");
		ExtraModuleNames.AddRange( new string[] { "HiGame" } );
	...
	internal static void ApplySharedHiGameTargetSettings(TargetRules Target)
	{
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			HiGameTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
		{
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Log.TraceWarning("HiGameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}
