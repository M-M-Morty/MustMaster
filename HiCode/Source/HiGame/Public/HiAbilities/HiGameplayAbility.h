...
public:
	...
	/** Ability Data */
	// To store data for specific functional abilities, avoiding the addition of too many attributes in the ability base class.
	UPROPERTY(BlueprintReadOnly, EditDefaultsOnly, Instanced, Category="Default|Data")
	TArray<TObjectPtr<UHiAbilityDataBase>> AbilityData;
...
protected:
	...
	// ability function end

public:
	UFUNCTION(BlueprintCallable, Category="Ability|Data")
	UHiAbilityDataBase* FindAbilityDataByClass(const TSubclassOf<UHiAbilityDataBase> AbilityDataClass) const;

	template<typename T>
	T* FindAbilityDataByClass() const
	{
		return (T*) FindAbilityDataByClass(T::StaticClass());
	}
};
