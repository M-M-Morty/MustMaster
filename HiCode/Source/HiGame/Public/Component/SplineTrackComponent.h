...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FHiOnSplineTriggerActorCreated, AActor*, Trigger);
...
public:
	...
	AActor * CreateTriggerActor(const FTransform& Transform=FTransform::Identity);
	...
	FHiOnSplineTriggerActorCreated OnTriggerCreated;
...
private:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Trigger Collision", meta=(AllowPrivateAccess="true"))
	TSubclassOf<AActor> StartTriggerClass;

	UPROPERTY(Transient)
	TObjectPtr<AActor> HeadTrigger = nullptr;

	UPROPERTY(Transient)
	TObjectPtr<AActor> TailTrigger = nullptr;
