...
public:
	...
	virtual void Process_Implementation(const float DeltaTime, const FPostProcessEvaluateContext& ViewContext) override;
...
private:
	void ProcessPerspectiveEffect(const float DeltaTime, const FPostProcessEvaluateContext& ViewContext, const FOcclusionDetectContext& DetectContext);

	void ProcessOutlineEffect(const float DeltaTime, const FPostProcessEvaluateContext& ViewContext, const FOcclusionDetectContext& DetectContext);
...
public:
	/* Other Settings */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Camera Occlusion|Settings")
	FName InvalidEffectCurveName;

private:
	/* Other Variables */
	UPROPERTY()
	bool bIsEffectDisabledByCurve = false;
