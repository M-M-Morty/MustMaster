...
#pragma optimize( "", off )
#include UE_INLINE_GENERATED_CPP_BY_NAME(HiAnimNode_LayeredBoneBlend)

#define DEFAULT_SOURCEINDEX 0xFF
/////////////////////////////////////////////////////
// FHiAnimNode_LayeredBoneBlend

FHiAnimNode_LayeredBoneBlend::FHiAnimNode_LayeredBoneBlend()
...
void FHiAnimNode_LayeredBoneBlend::Initialize_AnyThread(const FAnimationInitializeContext& Context)
{
...
void FHiAnimNode_LayeredBoneBlend::SetBasePoseLinkNode(FAnimNode_Base* NewLinkNode)
{
	BasePose.SetLinkNode(NewLinkNode);
}

void FHiAnimNode_LayeredBoneBlend::SetBasePoseDynamicLinkNode(FPoseLinkBase* InPoseLink)
{
	BasePose.SetDynamicLinkNode(InPoseLink);
}

int32 FHiAnimNode_LayeredBoneBlend::AddPose(FAnimNode_Base* NewLinkNode, float BlendWeight, UBlendProfile* BlendProfile)
{
	new (BlendPoses) FPoseLink();
	new (LayerSetup) FInputBlendPose();
	BlendWeights.Add(BlendWeight);
	BlendMasks.Add(BlendProfile);
	const int32 PoseIndex = BlendPoses.Num()-1;
	if (NewLinkNode)
	{
		BlendPoses[PoseIndex].SetLinkNode(NewLinkNode);
	}
	return PoseIndex;
}

int32 FHiAnimNode_LayeredBoneBlend::AddPose(FAnimNode_Base* NewLinkNode, float BlendWeight, FInputBlendPose BlendPose)
{
	int32 PoseIndex= AddPose(NewLinkNode, BlendWeight);
	if(LayerSetup.IsValidIndex(PoseIndex))
	{
		LayerSetup[PoseIndex] = BlendPose;
	}
	return PoseIndex;
}

void FHiAnimNode_LayeredBoneBlend::RebuildPerBoneBlendWeights(const USkeleton* InSkeleton)
{
	if (InSkeleton)
	{
...
bool FHiAnimNode_LayeredBoneBlend::ArePerBoneBlendWeightsValid(const USkeleton* InSkeleton) const
{
	return (InSkeleton != nullptr && InSkeleton->GetGuid() == SkeletonGuid && InSkeleton->GetVirtualBoneGuid() == VirtualBoneGuid);
}

void FHiAnimNode_LayeredBoneBlend::UpdateCachedBoneData(const FBoneContainer& RequiredBones, const USkeleton* Skeleton)
{
	...
	for (int32 RequiredBoneIndex=0; RequiredBoneIndex<NumRequiredBones; RequiredBoneIndex++)
	{
		...
		if (ensure(SkeletonBoneIndex != INDEX_NONE) && PerBoneBlendWeights.IsValidIndex(SkeletonBoneIndex))
		{
	...
	if (TotalCount > 0)
	{
		...
		for (int32 UIDIndex = 0; UIDIndex < CurveUIDCount; ++UIDIndex)
		{
			...
			if (CurrentPoseIndex != MAX_uint16 && CurvePoseSourceIndices.IsValidIndex(CurrentPoseIndex))
			{
				...
				if (CurveMetaData)
				{
					...
					for (int32 LinkedBoneIndex = 0; LinkedBoneIndex < LinkedBones.Num(); ++LinkedBoneIndex)
					{
						...
						{
							const int32 iCompactPoseIndex = CompactPoseIndex.GetInt();
							if (DesiredBoneBlendWeights.IsValidIndex(iCompactPoseIndex) && DesiredBoneBlendWeights[iCompactPoseIndex].BlendWeight > 0.f)
							{
								CurvePoseSourceIndices[CurrentPoseIndex] = DesiredBoneBlendWeights[iCompactPoseIndex].SourceIndex;
							}
						}
					}
				}
...
void FHiAnimNode_LayeredBoneBlend::CacheBones_AnyThread(const FAnimationCacheBonesContext& Context)
{
...
void FHiAnimNode_LayeredBoneBlend::Update_AnyThread(const FAnimationUpdateContext& Context)
{
...
void FHiAnimNode_LayeredBoneBlend::Evaluate_AnyThread(FPoseContext& Output)
{
...
void FHiAnimNode_LayeredBoneBlend::GatherDebugData(FNodeDebugData& DebugData)
{
...
void FHiAnimNode_LayeredBoneBlend::PreUpdate(const UAnimInstance* InAnimInstance)
{
	TArray<FAnimNode_Base*> LinkNodes;
	LinkNodes.AddUnique(BasePose.GetLinkNode());
	for (FPoseLink Pose : BlendPoses)
	{
		LinkNodes.AddUnique(Pose.GetLinkNode());
	}
	for (FAnimNode_Base*Node : LinkNodes)
	{
		if (Node && Node->HasPreUpdate())
		{
...
void FHiAnimNode_LayeredBoneBlend::SetBlendMask(int32 InPoseIndex, UBlendProfile* InBlendMask)
{
