...
FGameplayAbilitySpecHandle UHiAbilitySystemComponent::GetActiveAnimatingAbilityHandle()
{
	UGameplayAbility* AbilityCDO = GetAnimatingAbility();
	if(IsValid(AbilityCDO))
	{
		return FindAbilitySpecHandleFromClass(AbilityCDO->GetClass());
	}
	return FGameplayAbilitySpecHandle();
}

UGameplayAbility* UHiAbilitySystemComponent::GetAbilityInstanceFromAbilityHandle(
