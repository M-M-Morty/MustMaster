...
void UHiVehicleMovementComponent::UpdateRotationOnGround(float DeltaTime)
{
	...
	float TargetYaw = Rotation.Yaw + deltaYaw;
	Rotation.Yaw = FMath::FInterpTo(Rotation.Yaw, TargetYaw, DeltaTime, YawInterpSpeed);;
	float Pitch = 0;
	...
	Rotation.Roll = 0;
	VisibilityPitch = FMath::FInterpTo(VisibilityPitch, Pitch, DeltaTime, PitchInterpSpeed);
