...
void UHiGlideComponent::ProcessGlideRotation_Implementation(float DeltaTime)
{
	...
	if (Acceleration.IsNearlyZero())
	{
		...
		PreTargetRotation = FRotator::ZeroRotator;
		return;
	}
	...
	FRotator LocalTargetRotation = Owner->GetActorRotation().UnrotateVector(TargetRotation.Vector()).Rotation();
	if (FMath::Abs(PreTargetRotation.Yaw) > FMath::Abs(LocalTargetRotation.Yaw)
		&& FMath::Abs(LocalTargetRotation.Yaw) < MinTurnYaw && PreTargetRotation.Yaw*LocalTargetRotation.Yaw > 0.0f)
	{
		AccelerationDirection = 0.0f;
	}
	PreTargetRotation = LocalTargetRotation;
	
	if (FMath::Abs(LocalTargetRotation.Yaw) < MinTurnYaw)
	{
		GlideState = EHiGlideState::GlideForward;
	}
	else if (LocalTargetRotation.Yaw > 0.0f)
	{
