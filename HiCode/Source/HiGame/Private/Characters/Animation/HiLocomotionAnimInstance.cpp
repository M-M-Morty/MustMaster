...
void UHiLocomotionAnimInstance::OnUpdateComponent()
{
	...
	UHiGlideComponent* NewGlideComponent = Character->FindComponentByClass<UHiGlideComponent>();
	if (NewGlideComponent != GlideComponent)
	{
...
void UHiLocomotionAnimInstance::UpdateAnimatedYawOffset(float DeltaSeconds)
{
	...
	if(bSkipUpdateAnimatedYawOffset)
	{
		PreviousAnimatedYawOrientation = CurrentCharacterYaw;
		PreviousAnimatedYawOffsetDecline = RemainingReverseYawTime = YawRestoreDecayAlpha = 0.f;
		bSkipUpdateAnimatedYawOffset = false;
		return;
	}
	
	// Calculate yaw
	float TargetYawOffset = CalculateTargetYawOffset(CurrentCharacterYaw);
	...
	if (FMath::Abs(TargetYawOffset) < MaximumAngelForYawSmoothRestore)
	{
		...
		PreviousAnimatedYawOrientation = FMath::Wrap(CurrentCharacterYaw, 0.0f, 360.0f);
		return;
	}
...
void UHiLocomotionAnimInstance::SetSkipUpdateAnimatedYawOffset()
{
	bSkipUpdateAnimatedYawOffset = true;
}
