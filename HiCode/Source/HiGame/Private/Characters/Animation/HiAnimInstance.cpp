...
void UHiAnimInstance::NativeInitializeAnimation()
{
	...
	OnMontageStarted.AddUniqueDynamic(this, &UHiAnimInstance::OnMontageStartToPlay);
	OnMontageEnded.AddUniqueDynamic(this, &UHiAnimInstance::OnMontageEnd);
}
...
void UHiAnimInstance::OnMontageEnd(UAnimMontage* Montage, bool bInterrupted)
{
	UE_LOG(HIAnimInstanceLog, Log, TEXT("[%s]OnMontageEnd %s, bInterrupted:%d"), *GetName(), *Montage->GetPathName(), bInterrupted);
	if(UHiSkeletalMeshComponent* MeshComponent = Cast<UHiSkeletalMeshComponent>(GetSkelMeshComponent()))
	{
		MeshComponent->OnMontageEnd(this, Montage, bInterrupted);
	}
}
