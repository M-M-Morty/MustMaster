...
#include "VisionerCameraManager.h"
#include "VisionerNodes/VisionerNode_Slot.h"
...
UHiCameraOcclusionPostProcessor::UHiCameraOcclusionPostProcessor(class FObjectInitializer const& ObjectInitializer)
	...
	, InvalidEffectCurveName(FVisionerNode_Slot::DefaultOverrideCurveName)
{
}
...
void UHiCameraOcclusionPostProcessor::Process_Implementation(const float DeltaTime, const FPostProcessEvaluateContext& ViewContext)
{
	...
	if (AVisionerCameraManager* VisionerCameraManager = Cast<AVisionerCameraManager>(ViewContext.PlayerCameraManager))
	{
		if (UVisionerInstance* VisionerInstance = VisionerCameraManager->GetVisionerInstance())
		{
			float CurveValue = VisionerInstance->GetCachedCurveValue(InvalidEffectCurveName);
			bIsEffectDisabledByCurve = (CurveValue > UE_KINDA_SMALL_NUMBER);
		}
	}
...
void UHiCameraOcclusionPostProcessor::ProcessPerspectiveEffect(const float DeltaTime, const FPostProcessEvaluateContext& ViewContext, const FOcclusionDetectContext& DetectContext)
{
	...
	if (DetectContext.bVisiblityHit && bEnableOcclusionPerspective && (!bIsEffectDisabledByCurve))
	{
...
void UHiCameraOcclusionPostProcessor::ProcessOutlineEffect(const float DeltaTime, const FPostProcessEvaluateContext& ViewContext, const FOcclusionDetectContext& DetectContext)
{
	UWorld* World = GetWorld();
	check(World);

	bool bIsOutlineInEffectNew = bEnableOcclusionOutline && (!bIsEffectDisabledByCurve) && DetectContext.bVisiblityHit;
