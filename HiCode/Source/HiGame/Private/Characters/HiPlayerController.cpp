...
#include "TSF4GClientSubsystem.h"
#endif
#include "DistributedDSConnectionBase.h"
...
{
	Super::BeginPlay();
	if (Aether::GetSSInstanceType() == ESSInstanceType::Client)
	{
#if !PLATFORM_LINUX && !UE_SERVER
		UTSF4GClientSubsystem* ClientSubsystem = GetGameInstance()->GetSubsystem<UTSF4GClientSubsystem>();
		if (ClientSubsystem != nullptr)
		{
			ClientSubsystem->OnEnterDedicatedServer();
		}
#endif
	}
}

void AHiPlayerController::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (Aether::GetSSInstanceType() == ESSInstanceType::Client)
	{
		...
		if (ClientSubsystem != nullptr)
		{
			ClientSubsystem->OnLeaveDedicatedServer();
		}
#endif
	}
}
...
void AHiPlayerController::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(AHiPlayerController, SwitchPlayers, COND_AutonomousOnly);
	DOREPLIFETIME_CONDITION(AHiPlayerController, bIsReconnected, COND_OwnerOnly);
}
...
void AHiPlayerController::PawnLeavingGame()
{
	UnPossess();
}

void AHiPlayerController::InitPlayerState()
{
	if ( GetNetMode() != NM_Client )
	{
		UWorld* const World = GetWorld();
		AGameModeBase* GameMode = World ? World->GetAuthGameMode() : NULL;
		...
		if (GameMode == NULL)
		{
			...
			const AGameModeBase* DefaultGameMode = GameState ? GameState->GetDefaultGameMode() : NULL;
			if (DefaultGameMode != nullptr)
			{
				InitPlayerStateFromGameMode(DefaultGameMode);
			}
			return;
		}
		
		AHiGameModeBase* HiGameModeBase = Cast<AHiGameModeBase>(GameMode);
		if (HiGameModeBase == nullptr)
		{
			UE_LOG(LogHiGame, Error, TEXT("AHiPlayerController::InitPlayerState GameMode(%s) Type is not AHiGameModeBase, init default PlayerSteate"), *GameMode->GetName());
			InitPlayerStateFromGameMode(GameMode);
			return;
		}
		
		uint64 EntityID = PlayerProxyID;
		APlayerState* InactivePlayerState = HiGameModeBase->PopInactivePlayer(EntityID);
		if (InactivePlayerState == nullptr)
		{
			UE_LOG(LogHiGame, Log, TEXT("AHiPlayerController::InitPlayerState: Controller(%s) is new. spawn PlayerState"), *GetName());
			InitPlayerStateFromGameMode(GameMode);
			return;
		}
		UE_LOG(LogHiGame, Log, TEXT("AHiPlayerController::InitPlayerState: Controller(%s) reconnected. use EntityID(%llu)"), *GetName(), EntityID);
		PlayerState = InactivePlayerState;
		PlayerState->SetOwner(this);
		PlayerState->OnReactivated();
		bIsReconnected = true;
	}
}

void AHiPlayerController::InitPlayerStateFromGameMode(const AGameModeBase* GameMode)
{
	UWorld* const World = GetWorld();
	...
	{
		UE_LOG(LogHiGame, Log, TEXT("AController::InitPlayerStateFromGameMode: the PlayerStateClass of game mode %s is null, falling back to APlayerState."), *GameMode->GetName());
