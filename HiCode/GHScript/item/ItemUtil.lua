...
---@field task_item_display_type integer
---@field task_item_details string[]
...
function ItemUtil.GetItemManager(WorldContextObject)
    return UE.UGameplayStatics.GetPlayerController(WorldContextObject, 0).PlayerState.ItemManager
end
...
function ItemUtil.GetAllPhoneCards(WorldContextObject)
    ...
    for _, v in ipairs(TaskItems) do
        ...
        if ItemConfig.task_item_display_type == ItemDef.TASK_ITEM_DISPLAY_TYPE.PHONE_CARD then
