...
#include "Component/HiAILocomotionAppearance.h"
...
UHiAILocomotionAppearance::UHiAILocomotionAppearance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UHiAILocomotionAppearance::InitializeComponent()
{
	Super::InitializeComponent();
	MyCharacterMovementComponent = Cast<UHiCharacterMovementComponent>(CharacterOwner->GetMovementComponent());
}

void UHiAILocomotionAppearance::TickLocomotion(float DeltaTime)
{
...
void UHiAILocomotionAppearance::RagdollStart()
{
	Super::RagdollStart();
}

void UHiAILocomotionAppearance::RagdollEnd()
{
	Super::RagdollEnd();
}

void UHiAILocomotionAppearance::BeginPlay()
{
	Super::BeginPlay();
...
void UHiAILocomotionAppearance::OnRotationModeChanged(EHiRotationMode PreviousRotationMode)
{
	Super::OnRotationModeChanged(PreviousRotationMode);
	MyCharacterMovementComponent->SetMovementSettings(GetTargetMovementSettings());
}

void UHiAILocomotionAppearance::OnSpeedScaleChanged(float PrevSpeedScale)
{
...
void UHiAILocomotionAppearance::UpdateCharacterMovement()
{
...
void UHiAILocomotionAppearance::UpdateCharacterRotation(float DeltaTime)
{
	...
	case EHiRotationMode::LookingDirection:
		NewTargetOrientation = { 0, AimingRotation.Yaw + GetAnimCurveValue(NAME_YawOffset), 0.0f };
		break;
...
EHiGait UHiAILocomotionAppearance::GetActualGait(EHiGait AllowedGait) const
{
